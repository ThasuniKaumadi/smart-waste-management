...
export default function ResidentDashboardPage() {
  ...
  const upcoming = schedules.slice(0, 6)   // sidebar list
  const restScheds = schedules.slice(1, 4)   // below-hero upcoming previews
    ...
    >
      <style>{`
        ...
        .card {
          ...
          box-shadow:0 8px 32px -8px rgba(24,28,34,0.08);
          border:1px solid rgba(0,0,0,0.05); overflow:hidden;
        }
        .secondary-action {
          display:flex; align-items:center; gap:12px;
          padding:14px 16px; border-radius:12px; border:1px solid rgba(0,69,13,0.07);
        ...
        .status-pill {
          ...
          letter-spacing:0.07em; padding:3px 9px; border-radius:99px;
          font-family:'Manrope',sans-serif; white-space:nowrap; flex-shrink:0;
        }
        /* upcoming row in sidebar */
        .sched-row {
          display:flex; align-items:flex-start; gap:12px;
          padding:12px 18px; transition:background 0.15s;
        }
        ...
        .s1{animation:fadeUp .4s ease .04s both}
        .s2{animation:fadeUp .4s ease .09s both}
        .s3{animation:fadeUp .4s ease .14s both}
        .s4{animation:fadeUp .4s ease .19s both}
      ...
      {loading ? (
        ...
        <div className="outer-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                            ...
                            <div>
                              <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif' }}>
                                Next Collection
                              </p>
                              {rel.isToday && (
                                <span style={{ fontSize: 10, fontWeight: 700, background: '#00450d', color: 'white', padding: '1px 8px', borderRadius: 99, fontFamily: 'Manrope,sans-serif', letterSpacing: '0.06em' }}>
                                  TODAY
                                </span>
                              )}
                              {rel.isTmrw && (
                                <span style={{ fontSize: 10, fontWeight: 700, background: '#1d4ed8', color: 'white', padding: '1px 8px', borderRadius: 99, fontFamily: 'Manrope,sans-serif', letterSpacing: '0.06em' }}>
                                  TOMORROW
                                </span>
                              )}
                            </div>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                            <span style={{ fontSize: 13, fontWeight: 600, padding: '5px 14px', borderRadius: 99, background: wc.bg, color: wc.color, fontFamily: 'Manrope,sans-serif' }}>
                              {wc.label}
                            </span>
                            <span style={{ fontSize: 13, fontWeight: 600, padding: '5px 14px', borderRadius: 99, background: '#f4f6f3', color: '#374151', fontFamily: 'Inter,sans-serif', display: 'flex', alignItems: 'center', gap: 4 }}>
                              <span className="material-symbols-outlined" style={{ fontSize: 13 }}>schedule</span>
                              {nextSched.collection_time}
                            </span>
                            <span style={{ fontSize: 13, fontWeight: 600, padding: '5px 14px', borderRadius: 99, background: '#f4f6f3', color: '#374151', fontFamily: 'Inter,sans-serif' }}>
                              {FREQUENCIES[nextSched.frequency] || nextSched.frequency}
                            </span>
                          </div>

                          {nextSched.notes && (
                            <p style={{ marginTop: 10, fontSize: 12, color: '#717a6d', display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <Link href="/dashboard/resident/schedule"
                          style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '18px 22px', borderRadius: 14, background: wc.bg, border: `1px solid ${wc.pill}`, textDecoration: 'none', transition: 'all .2s' }}>
            ...
            {/* ② Upcoming mini cards — next 3 after the hero */}
            {restScheds.length > 0 && (
              <div className="s2">
                <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', marginBottom: 10 }}>
                  Coming Up
                </p>
            ...
            {/* ③ Today alert — shown below schedule if today has collections */}
            ...
            <div className="s3">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { label: 'Track Vehicle', desc: 'Live GPS location', icon: 'near_me', href: '/dashboard/resident/track', color: '#1d4ed8', bg: 'rgba(29,78,216,0.07)' },
                  { label: 'File Complaint', desc: 'Submit a formal grievance', icon: 'feedback', href: '/dashboard/resident/complaints/new', color: '#b45309', bg: 'rgba(180,83,9,0.07)' },
                  ...
                  { label: 'View Schedule', desc: 'Full collection calendar', icon: 'calendar_month', href: '/dashboard/resident/schedule', color: '#00450d', bg: 'rgba(0,69,13,0.07)' },
                ].map(a => (
            ...
            {/* ⑤ Complaints & Reports side by side */}
              ...
              <div className="card" style={{ padding: '18px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                  ...
                  <Link href="/dashboard/resident/complaints"
                    style={{ fontSize: 11, fontWeight: 700, color: '#00450d', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 1, fontFamily: 'Manrope,sans-serif' }}>
            ...
            <div className="s5" style={{ background: '#f0fdf4', border: '1px solid rgba(0,69,13,0.1)', borderRadius: 14, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              ...
              <Link href="/dashboard/resident/track"
                style={{ background: '#00450d', color: 'white', padding: '7px 16px', borderRadius: 99, fontWeight: 700, fontSize: 12, textDecoration: 'none', fontFamily: 'Manrope,sans-serif', whiteSpace: 'nowrap' }}>
                Track Now
              </Link>
            </div>
          ...
          <div style={{ position: 'sticky', top: 24 }} className="s1">
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px 18px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Link href="/dashboard/resident/schedule"
                  style={{ fontSize: 11, fontWeight: 700, color: '#00450d', textDecoration: 'none', fontFamily: 'Manrope,sans-serif', display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    return (
                      <div key={s.id} className="sched-row" style={{ background: rel.isToday ? 'rgba(0,69,13,0.025)' : undefined }}>
                        ...
                        <div style={{ flexShrink: 0, width: 44, textAlign: 'center', paddingTop: 2 }}>
                          {rel.label ? (
                            <>
                              <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: rel.isToday ? '#00450d' : '#64748b', fontFamily: 'Manrope,sans-serif' }}>
                                {rel.label}
                              </p>
                            ...
                            <>
                              <p style={{ fontSize: 9, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', fontFamily: 'Manrope,sans-serif' }}>
                                {date.toLocaleDateString('en-GB', { weekday: 'short' })}
                              </p>
                              <p style={{ fontSize: 20, fontWeight: 800, color: '#181c22', fontFamily: 'Manrope,sans-serif', lineHeight: 1.1 }}>
                                {date.getDate()}
                              </p>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 700, color: '#181c22', fontFamily: 'Manrope,sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {wc.label}
                            </span>
                          </div>
                          ...
                          {rel.isToday && (
                            <span style={{ fontSize: 9, fontWeight: 700, background: '#00450d', color: 'white', padding: '1px 7px', borderRadius: 99, display: 'inline-block', marginTop: 4, fontFamily: 'Manrope,sans-serif', letterSpacing: '0.05em' }}>
                              TODAY
                            </span>
                          )}
                        </div>
              ...
              <div style={{ padding: '11px 18px', borderTop: '1px solid #f0f0f0', background: '#fafafa' }}>
                <Link href="/dashboard/resident/schedule"
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontSize: 12, fontWeight: 700, color: '#00450d', textDecoration: 'none', fontFamily: 'Manrope,sans-serif' }}>
