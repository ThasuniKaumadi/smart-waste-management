...
export default function DriverDashboardPage() {
  ...
  async function loadData() {
    ...
    setStats({
      ...
      skippedCollections: skippedCollections || 0,
    ...
    const { data: collections } = await supabase
      ...
      .select('*, collection_stops(address, is_commercial)')
      .eq('driver_id', user.id)
      .order('created_at', { ascending: false })
      .limit(4)
                ...
                <div className="flex flex-col items-center mb-6">
                  <div className="relative w-32 h-32 mb-3">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="font-headline font-extrabold text-2xl" style={{ color: '#181c22' }}>
                        {completionRate}%
                      </span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6 s3">
            {[
              ...
              <div key={m.label} className="bento-card p-6">
                ...
                <p className="font-headline font-extrabold text-3xl tracking-tight mb-1" style={{ color: '#181c22' }}>
                  {m.value}
                </p>
                <p className="font-label text-xs font-bold uppercase mb-1"
                  style={{ letterSpacing: '0.12em', color: '#94a3b8' }}>{m.label}</p>
                ...
                <div className="space-y-3">
                  {todayRoutes.map(r => (
                    <div key={r.id} className="flex items-center gap-4 p-4 rounded-xl"
                      ...
                      <span className="status-badge" style={routeStatusStyle(r.status)}>
                        {r.status}
                      </span>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {recentCollections.map(c => (
                    <div key={c.id} className="flex items-center gap-4 p-4 rounded-xl"
                      ...
                      <span className="status-badge" style={collectionStatusStyle(c.status)}>
                        {c.status}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: '#717a6d' }}>
                  Every collection stop is logged on Polygon Amoy · CMC 2026
                </p>
              </div>
            </div>
            <Link href="/dashboard/driver/routes"
