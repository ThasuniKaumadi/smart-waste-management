...
function IntakeLogPageContent() {
    ...
    async function handleSubmit() {
        ...
        if (isRecycler && !formData.material_type && !formData.is_rejected) { setSubmitError('Please select the material type'); return }
        if (isRecycler && !formData.grade && !formData.is_rejected) { setSubmitError('Please select the material grade'); return }
        ...
        try {
            ...
            // Log to blockchain if accepted
            if (!formData.is_rejected) {
                setBlockchainPending(true)
                try {
                    const txHash = await logDeliveryOnChain(
                        savedIntake.id,
                        formData.material_type || handoff.route?.waste_type || 'general'
                    )
                    if (txHash) {
                        // Save TX hash back to the intake record
                        await supabase
                            .from('waste_intake_logs')
                            .update({ tx_hash: txHash })
                            .eq('id', savedIntake.id)
                        savedIntake.tx_hash = txHash
                    }
                } catch (e) {
                    console.error('Blockchain logging failed (non-fatal):', e)
                }
                setBlockchainPending(false)
            }

            setCompletedIntake(savedIntake)
            setStep('success')
        } catch {
    ...
    return (
        ...
        .condition-card { border: 2px solid #e5e7eb; border-radius: 12px; padding: 14px 16px; cursor: pointer; transition: all 0.2s ease; font-size: 14px; font-family: 'Inter', sans-serif; }
        .condition-card:hover { border-color: #00450d; background: #f9fff9; }
        .condition-card.selected { border-color: #00450d; background: #f0fdf4; }
        .submit-btn { background: #00450d; color: white; border: none; border-radius: 12px; padding: 16px; width: 100%; font-family: 'Manrope', sans-serif; font-weight: 700; font-size: 15px; cursor: pointer; transition: all 0.2s ease; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .submit-btn:hover { background: #1b5e20; box-shadow: 0 8px 24px rgba(0,69,13,0.25); }
        .submit-btn:disabled { opacity: 0.6; cursor: not-allowed; }
        .reject-btn { background: #fef2f2; color: #ba1a1a; border: 1.5px solid rgba(186,26,26,0.2); border-radius: 12px; padding: 16px; width: 100%; font-family: 'Manrope', sans-serif; font-weight: 700; font-size: 15px; cursor: pointer; transition: all 0.2s ease; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .reject-btn:hover { background: #ffdad6; }
        .code-input { width: 52px; height: 64px; border: 2px solid #e5e7eb; border-radius: 12px; text-align: center; font-size: 28px; font-weight: 800; font-family: 'Manrope', sans-serif; color: #181c22; background: #fafafa; transition: all 0.2s ease; outline: none; }
        .code-input:focus { border-color: #00450d; background: white; box-shadow: 0 0 0 3px rgba(0,69,13,0.08); }
        .code-input.filled { border-color: #1b5e20; background: #f0fdf4; color: #00450d; }
        .tx-box { background: #f0fdf4; border: 1px solid rgba(0,69,13,0.15); border-radius: 10px; padding: 12px 16px; display: flex; align-items: flex-start; gap: 10px; }
        .tx-hash { font-family: 'Courier New', monospace; font-size: 11px; color: #00450d; word-break: break-all; line-height: 1.5; }
        .chain-spinner { width: 18px; height: 18px; border: 2px solid rgba(0,69,13,0.2); border-top-color: #00450d; border-radius: 50%; animation: spin .8s linear infinite; flex-shrink: 0; margin-top: 2px; }
                ...
                {loadingHandoff && (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px', background: 'white', borderRadius: '16px', boxShadow: '0 10px 40px -10px rgba(24,28,34,0.08)' }}>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ width: 28, height: 28, border: '2px solid #00450d', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .8s linear infinite', margin: '0 auto 12px' }} />
                    ...
                    <div className="fade-2">
                        {!showCodeEntry ? (
                            <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 10px 40px -10px rgba(24,28,34,0.08)', border: '1px solid rgba(0,69,13,0.04)', textAlign: 'center' }}>
                                ...
                                <p className="font-headline font-bold text-lg mb-2" style={{ color: '#181c22' }}>No arrival selected</p>
                                ...
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    <button onClick={() => router.push('/dashboard/intake')} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: '1.5px solid #e5e7eb', background: 'white', fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: '14px', cursor: 'pointer', color: '#181c22' }}>← Back</button>
                                    <button onClick={() => setShowCodeEntry(true)} style={{ flex: 1, padding: '14px', borderRadius: '12px', background: '#00450d', color: 'white', fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: '14px', cursor: 'pointer', border: 'none' }}>Enter Code Manually</button>
                                </div>
                            </div>
                        ) : (
                            <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 10px 40px -10px rgba(24,28,34,0.08)', border: '1px solid rgba(0,69,13,0.04)' }}>
                                <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                                    ...
                                    <p className="font-headline font-bold text-lg mb-2" style={{ color: '#181c22' }}>Enter 6-digit handoff code</p>
                                ...
                                {codeError && (
                                    <div style={{ background: '#fef2f2', border: '1px solid rgba(186,26,26,0.2)', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <p style={{ fontSize: '14px', color: '#ba1a1a', margin: 0 }}>{codeError}</p>
                                    </div>
                                )}
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    <button onClick={() => setShowCodeEntry(false)} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: '1.5px solid #e5e7eb', background: 'white', fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: '14px', cursor: 'pointer', color: '#181c22' }}>Cancel</button>
                    ...
                    <div className="fade-2">
                        ...
                        <div style={{ background: '#f0fdf4', border: '1px solid rgba(0,69,13,0.12)', borderRadius: '16px', padding: '24px', marginBottom: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                                <p className="font-headline font-bold" style={{ color: '#00450d', fontSize: '15px' }}>✅ Route Details</p>
                                <span style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', padding: '2px 8px', borderRadius: '99px', background: 'rgba(0,69,13,0.08)', color: '#00450d', fontFamily: 'Manrope, sans-serif' }}>Auto-filled</span>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                {[
                                    ...
                                    <div key={item.label}>
                                        <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#717a6d', fontFamily: 'Manrope, sans-serif', marginBottom: '2px' }}>{item.label}</p>
                                        <p style={{ fontSize: '14px', fontWeight: 600, color: '#181c22', margin: 0 }}>{item.value}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div style={{ background: 'white', borderRadius: '16px', padding: '32px', boxShadow: '0 10px 40px -10px rgba(24,28,34,0.08)', border: '1px solid rgba(0,69,13,0.04)', marginBottom: '16px' }}>
                            <h3 className="font-headline font-bold text-lg mb-6" style={{ color: '#181c22' }}>Intake Details</h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>

                                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '12px' }}>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Actual Quantity *</label>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Unit *</label>
                                ...
                                {isRecycler ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Material Type *</label>
                                            <select className="select-field" value={formData.material_type} onChange={e => setFormData({ ...formData, material_type: e.target.value })}>
                                                <option value="">Select material type</option>
                                                {MATERIAL_TYPES.map(m => <option key={m} value={m.toLowerCase()}>{m}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Material Grade *</label>
                                            <select className="select-field" value={formData.grade} onChange={e => setFormData({ ...formData, grade: e.target.value })}>
                                        ...
                                        {formData.material_type && formData.actual_quantity && (
                                            <div style={{ padding: '10px 14px', background: '#f0fdf4', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <span style={{ fontSize: '12px', color: '#41493e' }}>Rate: LKR {getRateForMaterial(formData.material_type)}/kg</span>
                                                <span style={{ fontSize: '14px', fontWeight: 700, color: '#00450d', fontFamily: 'Manrope, sans-serif' }}>Total: LKR {calculateAmount()}</span>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Processing Method *</label>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Waste Condition *</label>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Notes (Optional)</label>
                        ...
                        {submitError && (
                            <div style={{ background: '#fef2f2', border: '1px solid rgba(186,26,26,0.2)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span className="material-symbols-outlined" style={{ color: '#ba1a1a', fontSize: '18px' }}>error</span>
                                <p style={{ fontSize: '14px', color: '#ba1a1a', margin: 0 }}>{submitError}</p>
                            </div>
                        )}
                        ...
                        {formData.is_rejected ? (
                            <div style={{ background: '#fef2f2', border: '1.5px solid rgba(186,26,26,0.2)', borderRadius: '16px', padding: '24px', marginBottom: '16px' }}>
                                <p className="font-headline font-bold mb-4" style={{ color: '#ba1a1a' }}>Rejection Details</p>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Rejection Reason *</label>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope, sans-serif', marginBottom: '8px' }}>Notes</label>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                <button onClick={handleSubmit} disabled={submitting || blockchainPending} className="submit-btn">
                                    {submitting ? (
                                        ...
                                        <><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>check_circle</span>Confirm Intake</>
                                    )}
                                </button>
                    ...
                    <div className="fade-2">
                        <div style={{ background: 'white', borderRadius: '20px', padding: '48px', boxShadow: '0 10px 40px -10px rgba(24,28,34,0.08)', border: '1px solid rgba(0,69,13,0.04)', textAlign: 'center' }}>
                            <div style={{ width: '72px', height: '72px', background: completedIntake.is_rejected ? '#fef2f2' : '#f0fdf4', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
                                <span className="material-symbols-outlined" style={{ color: completedIntake.is_rejected ? '#ba1a1a' : '#00450d', fontSize: '40px' }}>
                            ...
                            <p style={{ fontSize: '14px', color: '#717a6d', marginBottom: '32px' }}>
                                {completedIntake.is_rejected
                                    ...
                                    : 'The waste intake has been recorded and logged on-chain.'}
                            </p>

                            {!completedIntake.is_rejected && (
                                <div style={{ background: '#f0fdf4', borderRadius: '12px', padding: '20px', marginBottom: '24px', textAlign: 'left' }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: completedIntake.tx_hash || completedIntake.total_amount ? '16px' : 0 }}>
                                        {[
                                            ...
                                            <div key={item.label}>
                                                <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#717a6d', fontFamily: 'Manrope, sans-serif', margin: '0 0 2px' }}>{item.label}</p>
                                                <p style={{ fontSize: '14px', fontWeight: 600, color: '#181c22', textTransform: 'capitalize', margin: 0 }}>{item.value}</p>
                                            </div>
                                        ))}
                                    </div>

                                    {isRecycler && completedIntake.total_amount && (
                                        <div style={{ paddingTop: '14px', borderTop: '1px solid rgba(0,69,13,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: completedIntake.tx_hash ? '14px' : 0 }}>
                                            <p style={{ fontSize: '13px', color: '#41493e', margin: 0 }}>Payment due to CMC</p>
                                            <p className="font-headline font-bold text-xl" style={{ color: '#00450d', margin: 0 }}>LKR {completedIntake.total_amount.toLocaleString()}</p>
                                        </div>
                                    )}

                                    {/* Blockchain TX hash */}
                                    {completedIntake.tx_hash && (
                                        <div style={{ borderTop: '1px solid rgba(0,69,13,0.1)', paddingTop: '14px' }}>
                                            <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#717a6d', fontFamily: 'Manrope, sans-serif', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                                <span className="material-symbols-outlined" style={{ fontSize: '13px', color: '#00450d' }}>link</span>
                                                Blockchain Transaction
                                            </p>
                                            <div className="tx-box">
                                                <span className="material-symbols-outlined" style={{ fontSize: '16px', color: '#00450d', flexShrink: 0, marginTop: 1 }}>verified</span>
                                                <div>
                                                    <p className="tx-hash">{completedIntake.tx_hash}</p>
                                                    <a
                                                        href={`https://amoy.polygonscan.com/tx/${completedIntake.tx_hash}`}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        style={{ fontSize: '11px', color: '#1d4ed8', textDecoration: 'none', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 3, marginTop: 4 }}>
                                                        View on Polygonscan
                                    ...
                                    {/* Blockchain pending */}
                                    {blockchainPending && !completedIntake.tx_hash && (
                                        <div style={{ borderTop: '1px solid rgba(0,69,13,0.1)', paddingTop: '14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <div className="chain-spinner" />
                                            <p style={{ fontSize: '13px', color: '#717a6d', margin: 0 }}>Writing to Polygon Amoy blockchain…</p>
                                        </div>
                                    )}
                                </div>
                            )}
