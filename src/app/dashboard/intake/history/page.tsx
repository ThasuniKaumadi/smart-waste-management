...
export default function IntakeHistoryPage() {
    ...
    async function loadData() {
        ...
        logs.forEach((log: any) => {
            ...
            if (!weekMap[key]) {
                weekMap[key] = { week: weekKey, district, waste_type: wasteType, total_quantity: 0, unit: log.unit || 'kg', intake_count: 0, rejection_count: 0 }
            }
    ...
    function gradeLabel(grade: string) {
        if (grade === 'grade_a') return { label: 'Grade A', color: '#00450d', bg: '#f0fdf4' }
        if (grade === 'grade_b') return { label: 'Grade B', color: '#d97706', bg: '#fefce8' }
        if (grade === 'grade_c') return { label: 'Grade C', color: '#ba1a1a', bg: '#fef2f2' }
        return { label: grade || '—', color: '#64748b', bg: '#f1f5f9' }
    }
    ...
    return (
        ...
        .tx-chip { display: inline-flex; align-items: center; gap: 4px; padding: 3px 8px; border-radius: 6px; background: #f0fdf4; border: 1px solid rgba(0,69,13,0.12); font-size: 10px; font-family: 'Courier New', monospace; color: #00450d; cursor: pointer; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-decoration: none; }
      ...
      `}</style>
            ...
            {loading ? (
                ...
                <>
                    {/* Summary */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '28px' }} className="s2">
                        {[
                            ...
                            { label: isRecycler ? 'Total Payable' : 'Rejections', value: isRecycler ? `LKR ${totalAmount.toLocaleString()}` : intakes.filter(i => i.is_rejected).length, icon: isRecycler ? 'payments' : 'cancel', color: isRecycler ? '#2e7d32' : '#ba1a1a' },
                        ].map(m => (
                            <div key={m.label} className="bento-card" style={{ padding: '20px', overflow: 'visible' }}>
                                ...
                                <p className="font-headline font-extrabold" style={{ fontSize: '22px', color: '#181c22', margin: '0 0 2px', lineHeight: 1 }}>{m.value}</p>
                                <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#94a3b8', fontFamily: 'Manrope, sans-serif', margin: 0 }}>{m.label}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }} className="s3">
                        <button onClick={() => setActiveTab('history')} className={`tab-btn ${activeTab === 'history' ? 'active' : ''}`}>
                            <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>history</span>All Records
                        </button>
                        <button onClick={() => setActiveTab('tonnage')} className={`tab-btn ${activeTab === 'tonnage' ? 'active' : ''}`}>
                            <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>bar_chart</span>Weekly Tonnage Report
                        </button>
                    </div>
                    ...
                    {activeTab === 'history' && (
                        <div className="bento-card s3">
                            <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                                <h3 className="font-headline font-bold" style={{ fontSize: '17px', color: '#181c22', margin: 0 }}>All Records</h3>
                            ...
                            {filtered.length === 0 ? (
                                <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                                    <span className="material-symbols-outlined" style={{ fontSize: '40px', color: '#d1d5db', display: 'block', marginBottom: 12 }}>inventory</span>
                                    <p className="font-headline font-bold" style={{ color: '#181c22', fontSize: '16px' }}>No records found</p>
                                </div>
                            ) : (
                                    ...
                                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        <thead>
                                            <tr style={{ background: '#f9f9ff' }}>
                                                {['Date', 'Vehicle', 'Waste Type', isRecycler ? 'Material / Grade' : 'Method', 'Quantity', 'Condition', 'Status', ...(isRecycler ? ['Amount'] : []), 'Blockchain'].map(h => (
                                                    <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#94a3b8', fontFamily: 'Manrope, sans-serif', whiteSpace: 'nowrap' }}>{h}</th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {filtered.map(log => {
                                                ...
                                                return (
                                                    <tr key={log.id} style={{ borderTop: '1px solid rgba(0,69,13,0.04)' }}>
                                                        ...
                                                        <td style={{ padding: '12px 16px' }}>
                                                            <p style={{ fontSize: '13px', fontWeight: 500, color: '#181c22', margin: 0 }}>{log.vehicle_number || '—'}</p>
                                                        </td>
                                                        <td style={{ padding: '12px 16px' }}>
                                                            <p style={{ fontSize: '13px', color: '#64748b', textTransform: 'capitalize', margin: 0 }}>{log.waste_type || '—'}</p>
                                                        </td>
                                                        <td style={{ padding: '12px 16px' }}>
                                                            {isRecycler ? (
                                                                <div>
                                                                    <p style={{ fontSize: '13px', color: '#181c22', textTransform: 'capitalize', margin: '0 0 3px' }}>{log.material_type || '—'}</p>
                                                                    {grade && log.grade && (
                                                                        <span className="status-badge" style={{ background: grade.bg, color: grade.color, fontSize: '9px' }}>{grade.label}</span>
                                                                    )}
                                                                </div>
                                                            ) : (
                                                                <p style={{ fontSize: '13px', color: '#64748b', textTransform: 'capitalize', margin: 0 }}>{log.processing_method || '—'}</p>
                                                            )}
                                                        </td>
                                                        <td style={{ padding: '12px 16px' }}>
                                                            <p style={{ fontSize: '13px', fontWeight: 700, color: '#181c22', fontFamily: 'Manrope, sans-serif', margin: 0 }}>
                                                                {log.actual_quantity} {log.unit}
                                                            </p>
                                                        </td>
                                                        <td style={{ padding: '12px 16px' }}>
                                                            {log.condition ? (
                                                                <span className="status-badge" style={conditionStyle(log.condition)}>{log.condition}</span>
                                                            ) : '—'}
                                                        </td>
                                                        <td style={{ padding: '12px 16px' }}>
                                                            <span className="status-badge" style={log.is_rejected ? { background: '#fef2f2', color: '#ba1a1a' } : { background: '#f0fdf4', color: '#00450d' }}>
                                                                {log.is_rejected ? 'Rejected' : 'Accepted'}
                                                            </span>
                                                        </td>
                                                        {isRecycler && (
                                                            <td style={{ padding: '12px 16px' }}>
                                                                <p style={{ fontSize: '13px', fontWeight: 700, color: '#00450d', fontFamily: 'Manrope, sans-serif', margin: 0 }}>
                    ...
                    {activeTab === 'tonnage' && (
                        <div>
                            <div className="bento-card" style={{ padding: '20px 24px', marginBottom: '20px' }}>
                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                                    <div>
                                        <p className="font-headline font-bold" style={{ fontSize: '17px', color: '#181c22', margin: '0 0 2px' }}>Weekly Tonnage Report</p>
                                        <p style={{ fontSize: '13px', color: '#717a6d', margin: 0 }}>Waste received per district by type — weekly breakdown</p>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <span className="material-symbols-outlined" style={{ color: '#717a6d', fontSize: '18px' }}>calendar_month</span>
                                        <select value={selectedWeek} onChange={e => setSelectedWeek(e.target.value)}
                                            style={{ border: '1.5px solid #e5e7eb', borderRadius: '10px', padding: '8px 14px', fontSize: '13px', fontFamily: 'Inter, sans-serif', color: '#181c22', background: 'white', outline: 'none', cursor: 'pointer' }}>
                            ...
                            {weeklyData.length === 0 ? (
                                <div className="bento-card" style={{ padding: '60px 24px', textAlign: 'center' }}>
                                    <span className="material-symbols-outlined" style={{ fontSize: '40px', color: '#d1d5db', display: 'block', marginBottom: 12 }}>bar_chart</span>
                                    <p className="font-headline font-bold" style={{ color: '#181c22', fontSize: '16px', marginBottom: 4 }}>No tonnage data yet</p>
                                    <p style={{ fontSize: '13px', color: '#94a3b8' }}>Log waste intakes to see weekly reports here.</p>
                                </div>
                            ) : (
                                        ...
                                        <div key={week} className="week-section">
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                    ...
                                                    <p className="font-headline font-bold" style={{ color: '#181c22', margin: 0 }}>Week of {formatWeekLabel(week)}</p>
                                                </div>
                                                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                                    <span style={{ fontSize: '12px', color: '#717a6d' }}>Total: <strong style={{ color: '#181c22' }}>{weekTotal.toFixed(1)} kg</strong></span>
                                                    {weekRejections > 0 && (
                                                        <span style={{ fontSize: '12px', color: '#ba1a1a', background: '#fef2f2', padding: '2px 10px', borderRadius: '99px', fontWeight: 700, fontFamily: 'Manrope, sans-serif' }}>{weekRejections} rejected</span>
                                                    )}
                                                </div>
                                            </div>
                                                    ...
                                                    <thead>
                                                        <tr style={{ background: '#f9f9ff' }}>
                                                            {['Facility / District', 'Waste Type', 'Quantity', 'Deliveries', 'Share'].map(h => (
                                                                <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#94a3b8', fontFamily: 'Manrope, sans-serif' }}>{h}</th>
                                                            ))}
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {rows.map((row, idx) => {
                                                            ...
                                                            return (
                                                                <tr key={idx} style={{ borderTop: '1px solid rgba(0,69,13,0.04)' }}>
                                                                    <td style={{ padding: '12px 16px' }}><p style={{ fontSize: '13px', fontWeight: 600, color: '#181c22', margin: 0 }}>{row.district}</p></td>
                                                                    ...
                                                                    <td style={{ padding: '12px 16px' }}><p className="font-headline font-bold" style={{ fontSize: '13px', color: '#181c22', margin: 0 }}>{row.total_quantity.toFixed(1)} {row.unit}</p></td>
                                                                    <td style={{ padding: '12px 16px' }}><p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>{row.intake_count}</p></td>
                                                                    <td style={{ padding: '12px 16px' }}>
                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                                            ...
                                                                            <span style={{ fontSize: '12px', fontWeight: 700, color: '#00450d', fontFamily: 'Manrope, sans-serif', minWidth: '36px' }}>{share.toFixed(0)}%</span>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                            )
                                                        ...
                                                        <tr style={{ background: '#f0fdf4', borderTop: '2px solid rgba(0,69,13,0.1)' }}>
                                                            <td style={{ padding: '10px 16px' }} colSpan={2}><p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: '#00450d', fontFamily: 'Manrope, sans-serif', letterSpacing: '0.1em', margin: 0 }}>Week Total</p></td>
                                                            <td style={{ padding: '10px 16px' }}><p className="font-headline font-extrabold" style={{ fontSize: '13px', color: '#00450d', margin: 0 }}>{weekTotal.toFixed(1)} kg</p></td>
                                                            <td style={{ padding: '10px 16px' }}><p style={{ fontSize: '13px', fontWeight: 700, color: '#00450d', margin: 0 }}>{rows.reduce((sum, r) => sum + r.intake_count, 0)}</p></td>
                                                            <td style={{ padding: '10px 16px' }}><p style={{ fontSize: '11px', fontWeight: 700, color: '#00450d', margin: 0 }}>100%</p></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
