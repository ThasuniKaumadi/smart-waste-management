...
const SUPERVISOR_NAV = [
    ...
    { label: 'Announcements', href: '/dashboard/supervisor/announcements', icon: 'campaign' },
]
...
export default function SupervisorAnnouncementsPage() {
    ...
    async function loadData() {
        ...
        // Fetch announcements for this district
    ...
    async function handleSubmit() {
        if (!title.trim() || !content.trim()) {
            showToastMsg('Please fill in title and content.')
            return
        }
        setSubmitting(true)
        const supabase = createClient()
        ...
        if (error) {
            showToastMsg('Failed to post announcement. Please try again.')
            console.error(error)
        } else {
            showToastMsg('Announcement posted successfully!')
            setTitle('')
            setContent('')
            setType('info')
            setTargetWard('all')
            setTargetRoles(['driver', 'supervisor'])
            setShowForm(false)
            await loadData()
        }
        setSubmitting(false)
    ...
    return (
        ...
        .text-input, .text-area, .select-input {
          border: 1.5px solid rgba(0,69,13,0.12); border-radius: 10px;
          padding: 10px 14px; font-size: 13px; font-family: 'Inter', sans-serif;
          outline: none; color: #181c22; background: white; width: 100%;
        ...
        .submit-btn {
          ...
          padding: 10px 20px; font-family: 'Manrope', sans-serif; font-weight: 700;
        ...
        .cancel-btn {
          ...
          border-radius: 10px; padding: 10px 20px; font-family: 'Manrope', sans-serif;
          font-weight: 700; font-size: 13px; cursor: pointer;
        }
        .announcement-row { padding: 20px 24px; border-bottom: 1px solid rgba(0,69,13,0.04); transition: background 0.15s; }
        .announcement-row:hover { background: #f9fdf9; }
        .announcement-row:last-child { border-bottom: none; }
        .badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 99px; font-size: 10px; font-weight: 700; font-family: 'Manrope', sans-serif; letter-spacing: 0.06em; text-transform: uppercase; }
      ...
      `}</style>
            ...
            <section className="s1" style={{ marginBottom: '32px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                    ...
                    <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px', background: showForm ? '#f1f5f9' : '#00450d', color: showForm ? '#64748b' : 'white', border: 'none', cursor: 'pointer', fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: '13px', transition: 'all 0.2s' }}>
            ...
            {showForm && (
                <div className="bento-card form-anim s2" style={{ marginBottom: '24px', padding: '24px' }}>
                    <h3 style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: '16px', color: '#181c22', margin: '0 0 20px' }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', paddingTop: '2px' }}>
                                {['driver', 'supervisor', 'resident', 'contractor'].map(role => (
                                    <span key={role} className={`role-chip ${targetRoles.includes(role) ? 'selected' : ''}`} onClick={() => toggleRole(role)}>
                                        {role}
                                    </span>
                                ))}
                            </div>
            ...
            <div className="bento-card s3">
                ...
                {loading ? (
                    ...
                    <div>
                        {filtered.map(a => {
                            ...
                            return (
                                <div key={a.id} className="announcement-row" style={{ opacity: a.is_active ? 1 : 0.6 }}>
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                                                ...
                                                {!a.is_active && (
                                                    <span className="badge" style={{ background: '#f1f5f9', color: '#94a3b8' }}>Inactive</span>
                                                )}
                                                {a.ward && (
                                                    <span className="badge" style={{ background: 'rgba(0,69,13,0.07)', color: '#00450d' }}>
                                                        {a.ward}
                                                    </span>
                                                )}
                                            </div>
                                            <p style={{ fontSize: '14px', fontWeight: 700, color: '#181c22', margin: '0 0 4px', fontFamily: 'Manrope, sans-serif' }}>
                                                {a.title}
                                            </p>
                                            <p style={{ fontSize: '13px', color: '#41493e', margin: '0 0 8px', lineHeight: 1.5 }}>
                                                {a.content}
                                            </p>
                                            ...
                                            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                                                <button
                                                    ...
                                                    <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>
                                                        {a.is_active ? 'visibility_off' : 'visibility'}
                                                    </span>
                ...
                <div style={{ padding: '12px 24px', borderTop: '1px solid rgba(0,69,13,0.06)', background: '#f9fdf9', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#717a6d', margin: 0 }}>
                        Announcements are visible to the selected roles in your district. You can only edit or delete your own posts.
                    </p>
                </div>
            </div>
        </DashboardLayout>
