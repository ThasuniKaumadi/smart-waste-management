...
interface Route {
    id: string; route_name: string; district: string; ward: string; vehicle_number: string
    date: string; status: string; shift: string; driver_id: string | null; schedule_id: string | null
    profiles: { full_name: string } | null
    collection_stops: { id: string; status: string; skip_reason: string | null }[]
    exception_alerts: { id: string; severity: string }[]
}
interface Stop {
    id: string; road_name: string; address: string; stop_order: number; status: string
    skip_reason: string | null; is_commercial: boolean; frequency: string | null
}
interface HistoryRoute {
    id: string; route_name: string; ward: string; date: string; status: string; shift: string
    driver_name: string | null; vehicle_number: string; total_stops: number
    completed_stops: number; skipped_stops: number; completion_pct: number; alert_count: number
}
...
export default function SupervisorRoutesPage() {
    ...
    async function loadData() {
        ...
        let query = supabase.from('routes')
            .select(`id, route_name, district, ward, vehicle_number, date, status, shift, driver_id, schedule_id, profiles:driver_id(full_name), collection_stops(id, status, skip_reason), exception_alerts(id, severity)`)
            .eq('district', p?.district || '').in('status', ['active', 'pending']).order('date', { ascending: false })
    ...
    async function loadHistory() {
        ...
        const { data: p } = await supabase.from('profiles').select('*').eq('id', user.id).single()
        const wards: string[] = p?.assigned_wards || []
        const since = new Date(); since.setDate(since.getDate() - historyDays)
        let query = supabase.from('routes')
            .select(`id, route_name, ward, date, status, shift, vehicle_number, profiles:driver_id(full_name), collection_stops(id, status), exception_alerts(id, is_resolved)`)
            .eq('district', p?.district || '').eq('status', 'completed')
            .gte('date', since.toISOString().split('T')[0]).order('date', { ascending: false }).limit(100)
        if (wards.length > 0) query = query.in('ward', wards)
        const { data } = await query
        setHistory((data || []).map((r: any) => {
            ...
            return { id: r.id, route_name: r.route_name, ward: r.ward, date: r.date, status: r.status, shift: r.shift, driver_name: r.profiles?.full_name || null, vehicle_number: r.vehicle_number, total_stops: stops.length, completed_stops: done, skipped_stops: skipped, completion_pct: stops.length > 0 ? Math.round((done / stops.length) * 100) : 0, alert_count: (r.exception_alerts || []).length }
        }))
        setHistoryLoading(false)
    }

    async function loadStops(routeId: string) {
        if (routeStops[routeId]) { setExpandedRoute(expandedRoute === routeId ? null : routeId); return }
        setLoadingStops(routeId)
        const supabase = createClient()
        const { data } = await supabase.from('collection_stops').select('*').eq('route_id', routeId).order('stop_order', { ascending: true })
        setRouteStops(prev => ({ ...prev, [routeId]: data || [] }))
        setExpandedRoute(routeId); setLoadingStops(null)
    }
    ...
    const histAvg = history.length > 0 ? Math.round(history.reduce((s, r) => s + r.completion_pct, 0) / history.length) : 0
    const histSkipped = history.reduce((s, r) => s + r.skipped_stops, 0)
    ...
    return (
        ...
        .route-row { padding:18px 24px; border-bottom:1px solid rgba(0,69,13,0.04); transition:background 0.2s; cursor:pointer; }
        .route-row:hover { background:#f9fdf9; }
        .route-row:last-child { border-bottom:none; }
        .history-row { padding:14px 24px; border-bottom:1px solid rgba(0,69,13,0.04); display:flex; align-items:center; gap:16px; transition:background 0.2s; }
        ...
        .ward-chip { background:white; border-radius:12px; padding:14px 18px; border:1px solid rgba(0,69,13,0.08); transition:all 0.2s; }
            ...
            <section style={{ marginBottom: '32px' }} className="s1">
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button className={`tab-btn ${activeTab === 'active' ? 'tab-active' : 'tab-inactive'}`} onClick={() => setActiveTab('active')}>
                            <span className="material-symbols-outlined" style={{ fontSize: '16px', verticalAlign: 'middle', marginRight: '4px' }}>route</span>Active Routes
                        </button>
                        <button className={`tab-btn ${activeTab === 'history' ? 'tab-active' : 'tab-inactive'}`} onClick={() => setActiveTab('history')}>
                            <span className="material-symbols-outlined" style={{ fontSize: '16px', verticalAlign: 'middle', marginRight: '4px' }}>history</span>History
                            {history.length > 0 && (
                                <span style={{ marginLeft: '6px', background: activeTab === 'history' ? 'rgba(255,255,255,0.25)' : '#00450d', color: 'white', borderRadius: '99px', fontSize: '10px', fontWeight: 700, padding: '1px 6px' }}>{history.length}</span>
                            )}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="bento-card s3">
                        <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '16px', color: '#181c22', margin: 0 }}>Active & Pending Routes</h3>
                        ...
                        {loading ? (
                            ...
                            <div style={{ textAlign: 'center', padding: '64px 24px' }}>
                                ...
                                <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '15px', color: '#181c22', margin: '0 0 6px' }}>No routes found</p>
                                            ...
                                            <div className="route-row" onClick={() => loadStops(route.id)}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                                    <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: ss.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                        <span className="material-symbols-outlined" style={{ color: ss.color, fontSize: '20px' }}>{route.status === 'active' ? 'directions_car' : 'schedule'}</span>
                                                    </div>
                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                                            ...
                                                            <span className="badge" style={{ background: ss.bg, color: ss.color }}>
                                                                <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: ss.dot, display: 'inline-block' }} />{route.status}
                                                            </span>
                                                            ...
                                                            {routeAlerts.length > 0 && (
                                                                <span className="badge" style={{ background: '#fef2f2', color: '#dc2626' }}>
                                                                    <span className="material-symbols-outlined" style={{ fontSize: '11px' }}>warning</span>{routeAlerts.length} alert{routeAlerts.length > 1 ? 's' : ''}
                                                                </span>
                                                            )}
                                                        </div>
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', fontSize: '12px', color: '#94a3b8' }}>
                                                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '13px' }}>location_on</span>{route.ward || route.district}</span>
                                                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '13px' }}>person</span>{(route.profiles as any)?.full_name || 'Unassigned'}</span>
                                                            {route.vehicle_number && <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '13px' }}>directions_car</span>{route.vehicle_number}</span>}
                                                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '13px' }}>calendar_today</span>{new Date(route.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                                        </div>
                                                        {route.status === 'active' && stops.length > 0 && (
                                                            <div style={{ marginTop: '10px' }}>
                                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                                                                    <span style={{ fontSize: '11px', color: '#717a6d' }}>{completedStop} of {stops.length} stops · {skippedStop} skipped</span>
                                                    ...
                                                    <span className="material-symbols-outlined" style={{ color: '#94a3b8', fontSize: '20px', flexShrink: 0, transition: 'transform 0.2s', transform: isExpanded ? 'rotate(180deg)' : 'none' }}>expand_more</span>
                                                </div>
                                            </div>
                                            {isExpanded && (
                                                    ...
                                                    {loadingStops === route.id ? (
                                                        ...
                                                        <div>
                                                            <div style={{ padding: '10px 24px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                <p style={{ fontSize: '11px', fontWeight: 700, color: '#717a6d', textTransform: 'uppercase', letterSpacing: '0.1em', fontFamily: 'Manrope,sans-serif', margin: 0 }}>{expandedStops.length} stops</p>
                        ...
                        <div style={{ padding: '12px 24px', borderTop: '1px solid rgba(0,69,13,0.06)', background: '#f9fdf9', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <p style={{ fontSize: '12px', color: '#717a6d', margin: 0 }}>Showing {filtered.length} of {routes.length} routes in your assigned wards</p>
                        </div>
                    </div>
                </>
            )}
            ...
            {activeTab === 'history' && (
                <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }} className="s1">
                        ...
                        {[7, 14, 30, 60].map(d => (
                            <button key={d} onClick={() => setHistoryDays(d)} className={`filter-btn ${historyDays === d ? 'active' : ''}`}>{d} days</button>
                        ))}
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '24px' }} className="s2">
                        {[
                            ...
                            { label: 'Total Skipped', value: histSkipped, icon: 'do_not_disturb', color: histSkipped > 0 ? '#d97706' : '#00450d', bg: histSkipped > 0 ? '#fefce8' : '#f0fdf4' },
                    ...
                    <div className="bento-card s3">
                        <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '16px', color: '#181c22', margin: 0 }}>Completed Routes</h3>
                        ...
                        {historyLoading ? (
                            ...
                            <div style={{ textAlign: 'center', padding: '64px 24px' }}>
                                ...
                                <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '15px', color: '#181c22', margin: '0 0 6px' }}>No completed routes</p>
                                    ...
                                    return (
                                        <div key={route.id} className="history-row">
                                            <div style={{ width: '52px', textAlign: 'center', flexShrink: 0 }}>
                                                <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 800, fontSize: '18px', color: '#181c22', margin: 0, lineHeight: 1 }}>{new Date(route.date).getDate()}</p>
                                                <p style={{ fontSize: '10px', color: '#94a3b8', margin: '2px 0 0', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{new Date(route.date).toLocaleDateString('en-GB', { month: 'short' })}</p>
                                            </div>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px', flexWrap: 'wrap' }}>
                                                    ...
                                                    {route.alert_count > 0 && <span className="badge" style={{ background: '#fef2f2', color: '#dc2626' }}>{route.alert_count} alert{route.alert_count > 1 ? 's' : ''}</span>}
                                                </div>
                                                <div style={{ display: 'flex', gap: '12px', fontSize: '11px', color: '#94a3b8', flexWrap: 'wrap' }}>
                                                    ...
                                                    {route.skipped_stops > 0 && <span style={{ color: '#dc2626' }}>· {route.skipped_stops} skipped</span>}
                                                </div>
                                            </div>
                                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                                <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 800, fontSize: '16px', color: pColor, margin: '0 0 4px', lineHeight: 1 }}>{route.completion_pct}%</p>
                                                ...
                                                <p style={{ fontSize: '10px', color: '#94a3b8', margin: '3px 0 0' }}>{route.completed_stops}/{route.total_stops} stops</p>
                                            </div>
                                        </div>
                                    )
                                })}
                        ...
                        <div style={{ padding: '12px 24px', borderTop: '1px solid rgba(0,69,13,0.06)', background: '#f9fdf9', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span className="material-symbols-outlined" style={{ color: '#00450d', fontSize: '15px' }}>info</span>
                            <p style={{ fontSize: '12px', color: '#717a6d', margin: 0 }}>Showing completed routes from the last {historyDays} days in your assigned wards</p>
                        </div>
                    </div>
                </>
            )}
