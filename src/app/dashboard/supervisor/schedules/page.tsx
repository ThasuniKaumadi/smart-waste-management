...
export default function SupervisorSchedulesPage() {
    ...
    async function loadData() {
        ...
        // Fetch published + cancelled schedules for this district
        ...
        // Filter to supervisor's assigned wards
        let filtered = schedData || []
        if (wards.length > 0) {
            filtered = filtered.filter((s: Schedule) => {
                const schedWards = s.wards?.length > 0 ? s.wards : s.ward ? [s.ward] : []
                // Include district-wide schedules (no wards) + schedules covering supervisor's wards
        ...
        if (filtered.length > 0) {
            ...
            const bySchedule: Record<string, Route[]> = {}
                ; (routesData || []).forEach((r: any) => {
                    ...
                    bySchedule[r.schedule_id].push({
                        id: r.id, route_name: r.route_name, ward: r.ward,
                        shift: r.shift, status: r.status, driver_id: r.driver_id,
                        vehicle_number: r.vehicle_number, driver_name: r.driver?.full_name || null,
                    })
                })
                ...
                {loading ? (
                    ...
                    <div className="card" style={{ padding: '60px 24px', textAlign: 'center' }}>
                        ...
                        <p style={{ fontSize: 13, color: '#94a3b8' }}>
                            {assignedWards.length > 0
                                ? `No upcoming schedules for ${assignedWards.join(', ')}`
                                : 'No upcoming schedules in your district'}
                        </p>
                    </div>
                ) : (
                    <div className="card">
                        {displayed.map(schedule => {
                            ...
                            // Streets relevant to this supervisor's wards
                            ...
                            return (
                                <div key={schedule.id}>
                                    <div className={`sched-row ${cancelled ? 'cancelled' : ''}`}
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                                            ...
                                            <div style={{ width: 40, height: 40, borderRadius: 12, background: cancelled ? '#fef2f2' : ws.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                <span className="msf-fill" style={{ fontSize: 20, color: cancelled ? '#ba1a1a' : ws.color }}>
                                                    {cancelled ? 'cancel' : ws.icon}
                                                </span>
                                            </div>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                                                    <span style={{ fontSize: 14, fontWeight: 700, color: cancelled ? '#94a3b8' : '#181c22', fontFamily: 'Manrope,sans-serif', textDecoration: cancelled ? 'line-through' : 'none', textTransform: 'capitalize' }}>
                                                        {ws.label}
                                                    </span>
                                                    {cancelled
                                                    ...
                                                    {schedRoutes.length > 0 && (
                                                        <span className="badge" style={{ background: '#eff6ff', color: '#1d4ed8' }}>
                                                            <span className="msf" style={{ fontSize: 11 }}>route</span>
                                                            {schedRoutes.length} route{schedRoutes.length !== 1 ? 's' : ''}
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 14px', fontSize: 12, color: '#717a6d' }}>
                                                    ...
                                                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                        ...
                                                        {schedule.collection_day} · {schedule.collection_time}
                                                    </span>
                                                    {schedWards.length > 0 && (
                                                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                    ...
                                                    {schedule.frequency && (
                                                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                            <span className="msf" style={{ fontSize: 13 }}>repeat</span>
                                                            {FREQUENCIES[schedule.frequency] || schedule.frequency}
                                                        </span>
                                                    )}
                                                </div>

                                                {/* Streets for supervisor's wards */}
                                                {relevantStreets.length > 0 && !cancelled && (
                                                    <div style={{ marginTop: 7, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                                                        {relevantStreets.map(street => (
                                                            <span key={street} className="street-chip">{street}</span>
                                                        ))}
                                                    </div>
                                                )}
                                    ...
                                    {isExpanded && (
                                        <div className="slide-down" style={{ padding: '12px 20px', background: '#f9fdf9', borderBottom: '1px solid rgba(0,69,13,0.05)' }}>
                                            {schedRoutes.length === 0 ? (
                                                ...
                                                <>
                                                    <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', marginBottom: 8 }}>
                                                        Routes — assigned by contractor
                                                    </p>
                                                        ...
                                                        <div key={route.id} className="route-row">
                                                            ...
                                                            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                                                <span className="badge" style={{ background: route.driver_id ? '#f0fdf4' : '#fef2f2', color: route.driver_id ? '#00450d' : '#ba1a1a' }}>
                                                                    <span className="msf" style={{ fontSize: 11 }}>person</span>
                                                                    {route.driver_name || 'No driver'}
                                                                </span>
                                                                {route.vehicle_number && (
                                                                    <span className="badge" style={{ background: '#f0fdf4', color: '#00450d' }}>
                                                                        <span className="msf" style={{ fontSize: 11 }}>local_shipping</span>
                                                                        {route.vehicle_number}
                                                                    </span>
                                                                )}
                                                            </div>
