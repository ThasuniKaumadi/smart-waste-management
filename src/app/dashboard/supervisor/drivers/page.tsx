...
interface DriverStat {
    id: string; full_name: string; phone: string | null; ward: string | null
    totalRoutes: number; completedRoutes: number; activeRoutes: number
    totalStops: number; completedStops: number; skippedStops: number
    openAlerts: number; completionRate: number; lastActive: string | null
}
...
export default function SupervisorDriversPage() {
    ...
    async function loadData() {
        ...
        let routeQuery = supabase.from('routes')
            .select(`id, status, driver_id, ward, date, profiles:driver_id(id, full_name, phone, ward), collection_stops(id, status), exception_alerts(id, is_resolved)`)
        ...
        for (const route of routeData || []) {
            ...
            if (!driverMap[driverId]) {
                driverMap[driverId] = { id: driverId, full_name: driverProfile.full_name || 'Unknown Driver', phone: driverProfile.phone || null, ward: driverProfile.ward || route.ward || null, totalRoutes: 0, completedRoutes: 0, activeRoutes: 0, totalStops: 0, completedStops: 0, skippedStops: 0, openAlerts: 0, completionRate: 0, lastActive: null }
            }
            ...
            d.openAlerts += ((route.exception_alerts as any[]) || []).filter(a => !a.is_resolved).length
            if (!d.lastActive || route.date > d.lastActive) d.lastActive = route.date
        }
        setDrivers(Object.values(driverMap).map(d => ({ ...d, completionRate: d.totalStops > 0 ? Math.round((d.completedStops / d.totalStops) * 100) : 0 })))
        setLoading(false)
    }
    ...
    function pColor(rate: number) {
        if (rate >= 85) return { color: '#00450d', bg: '#f0fdf4' }
        if (rate >= 65) return { color: '#d97706', bg: '#fefce8' }
        return { color: '#dc2626', bg: '#fef2f2' }
    }
    function pLabel(rate: number) {
        if (rate >= 85) return 'Excellent'
        if (rate >= 65) return 'Average'
        return 'Needs attention'
    }

    const avgCompletion = drivers.length > 0 ? Math.round(drivers.reduce((s, d) => s + d.completionRate, 0) / drivers.length) : 0
    const totalOpenAlerts = drivers.reduce((s, d) => s + d.openAlerts, 0)
    const needsAttention = drivers.filter(d => d.completionRate < 65 || d.openAlerts > 0).length
    ...
    return (
        ...
        .driver-row { padding:16px 24px; border-bottom:1px solid rgba(0,69,13,0.04); transition:background 0.2s; }
        .driver-row:hover { background:#f9fdf9; }
        .driver-row:last-child { border-bottom:none; }
        .sort-btn { display:inline-flex; align-items:center; gap:4px; font-size:11px; font-weight:700; color:#717a6d; font-family:'Manrope',sans-serif; text-transform:uppercase; letter-spacing:0.08em; cursor:pointer; border:none; background:none; padding:4px 8px; border-radius:6px; transition:all 0.15s; }
        .sort-btn:hover { background:rgba(0,69,13,0.06); color:#00450d; }
        .sort-btn.active { color:#00450d; background:rgba(0,69,13,0.08); }
        .search-input { border:1.5px solid rgba(0,69,13,0.1); border-radius:10px; padding:8px 14px; font-size:13px; font-family:'Inter',sans-serif; outline:none; color:#181c22; background:white; width:220px; transition:border 0.2s; }
        .search-input:focus { border-color:#00450d; }
        .badge { display:inline-flex; align-items:center; padding:3px 10px; border-radius:99px; font-size:10px; font-weight:700; font-family:'Manrope',sans-serif; letter-spacing:0.06em; text-transform:uppercase; white-space:nowrap; }
      ...
      `}</style>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '24px' }} className="s2">
                {[
                    ...
                    { label: 'Need Attention', value: needsAttention, icon: 'person_alert', color: needsAttention > 0 ? '#d97706' : '#00450d', bg: needsAttention > 0 ? '#fefce8' : '#f0fdf4' },
                ].map(m => (
            ...
            <div className="bento-card s3">
                <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '16px', color: '#181c22', margin: 0 }}>Driver List</h3>
                ...
                <div style={{ padding: '8px 24px', borderBottom: '1px solid rgba(0,69,13,0.04)', display: 'flex', alignItems: 'center', gap: '8px', background: '#fafdfb' }}>
                    <div style={{ flex: 2 }}>
                        <button className={`sort-btn ${sortBy === 'name' ? 'active' : ''}`} onClick={() => toggleSort('name')}>
                            Driver <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>{sortBy === 'name' ? (sortDir === 'asc' ? 'arrow_upward' : 'arrow_downward') : 'unfold_more'}</span>
                        </button>
                    </div>
                    {[
                        ...
                        <div key={col.key} style={{ flex: 1, textAlign: 'center' }}>
                            <button className={`sort-btn ${sortBy === col.key ? 'active' : ''}`} onClick={() => toggleSort(col.key)}>
                                {col.label} <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>{sortBy === col.key ? (sortDir === 'asc' ? 'arrow_upward' : 'arrow_downward') : 'unfold_more'}</span>
                            </button>
                        </div>
                    ))}
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '64px 24px' }}>
                        ...
                        <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '15px', color: '#181c22', margin: '0 0 6px' }}>
                    ...
                    <div>
                        {filtered.map(driver => {
                            const pc = pColor(driver.completionRate)
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <div style={{ flex: 2, display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: isActive ? '#00450d' : '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                                                <span style={{ fontSize: '13px', fontWeight: 700, color: isActive ? 'white' : '#9ca3af', fontFamily: 'Manrope,sans-serif' }}>{driver.full_name.charAt(0).toUpperCase()}</span>
                                                {isActive && <span style={{ position: 'absolute', bottom: 0, right: 0, width: '10px', height: '10px', borderRadius: '50%', background: '#22c55e', border: '2px solid white' }} />}
                                            </div>
                                            <div>
                                                <p style={{ fontSize: '13px', fontWeight: 700, color: '#181c22', fontFamily: 'Manrope,sans-serif', margin: '0 0 2px' }}>{driver.full_name}</p>
                                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                                                    {driver.ward && <span style={{ fontSize: '11px', color: '#94a3b8' }}>{driver.ward}</span>}
                                                    {driver.phone && <span style={{ fontSize: '11px', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '2px' }}><span className="material-symbols-outlined" style={{ fontSize: '11px' }}>phone</span>{driver.phone}</span>}
                                                    <span style={{ fontSize: '11px', color: '#94a3b8' }}>{driver.totalRoutes} route{driver.totalRoutes !== 1 ? 's' : ''} · {driver.totalStops} stops</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div style={{ flex: 1, textAlign: 'center' }}>
                                            <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 800, fontSize: '18px', color: pc.color, margin: '0 0 4px', lineHeight: 1 }}>{driver.completionRate}%</p>
                                            ...
                                            <p style={{ fontSize: '10px', color: '#94a3b8', margin: '4px 0 0' }}>{driver.completedStops}/{driver.totalStops}</p>
                                        </div>
                                        <div style={{ flex: 1, textAlign: 'center' }}>
                                            <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '18px', color: driver.skippedStops > 5 ? '#dc2626' : driver.skippedStops > 0 ? '#d97706' : '#00450d', margin: 0, lineHeight: 1 }}>{driver.skippedStops}</p>
                                            <p style={{ fontSize: '10px', color: '#94a3b8', margin: '4px 0 0' }}>skipped</p>
                                        </div>
                                        <div style={{ flex: 1, textAlign: 'center' }}>
                                            <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '18px', color: driver.openAlerts > 0 ? '#dc2626' : '#00450d', margin: 0, lineHeight: 1 }}>{driver.openAlerts}</p>
                                            <p style={{ fontSize: '10px', color: '#94a3b8', margin: '4px 0 0' }}>open alerts</p>
                                        </div>
                                        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
                                            <span className="badge" style={{ background: pc.bg, color: pc.color }}>{pLabel(driver.completionRate)}</span>
                                        </div>
                                    </div>
                                    <div style={{ marginTop: '8px', paddingLeft: '48px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                                        {driver.lastActive && <span style={{ fontSize: '11px', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '12px' }}>calendar_today</span>Last active: {new Date(driver.lastActive).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>}
                                        {driver.completedRoutes > 0 && <span style={{ fontSize: '11px', color: '#00450d', display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '12px' }}>check_circle</span>{driver.completedRoutes} completed</span>}
                                        {driver.activeRoutes > 0 && <span style={{ fontSize: '11px', color: '#1d4ed8', display: 'flex', alignItems: 'center', gap: '3px' }}><span className="material-symbols-outlined" style={{ fontSize: '12px' }}>directions_car</span>{driver.activeRoutes} active now</span>}
                                    </div>
                                </div>
                            )
                        })}
