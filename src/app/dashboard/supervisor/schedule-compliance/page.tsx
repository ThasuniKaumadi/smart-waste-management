...
interface ComplianceRow {
    scheduleId: string; scheduledDate: string; wasteType: string; collectionTime: string
    wards: string[]; scheduled: number; collected: number; skipped: number
    compliancePct: number; status: 'compliant' | 'partial' | 'missed' | 'upcoming'
}
...
export default function ScheduleCompliancePage() {
    ...
    async function loadData() {
        ...
        let routeQuery = supabase.from('routes')
            .select(`id, schedule_id, ward, date, status, collection_stops(id, status)`)
        ...
        const complianceRows: ComplianceRow[] = (schedules || []).map((s: any) => {
            ...
            const schedDate = new Date(s.scheduled_date); schedDate.setHours(0, 0, 0, 0)
            ...
            return {
                scheduleId: s.id, scheduledDate: s.scheduled_date,
                wasteType: s.waste_type || 'general', collectionTime: s.collection_time || '',
                wards: s.wards || (s.ward ? [s.ward] : []),
                scheduled: totalStops, collected: collectedStops, skipped: skippedStops,
    ...
    const pastRows = rows.filter(r => r.status !== 'upcoming')
    const overallPct = pastRows.length > 0
        ? Math.round(pastRows.reduce((s, r) => s + (r.compliancePct >= 0 ? r.compliancePct : 0), 0) / pastRows.length)
        : null
            ...
            <section style={{ marginBottom: '32px' }} className="s1">
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        {weekOffset !== 0 && <button className="week-btn" onClick={() => setWeekOffset(0)}>Today</button>}
                    </div>
                </div>
            ...
            <div className="bento-card s3">
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '64px 24px' }}>
                        ...
                        <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '15px', color: '#181c22', margin: '0 0 6px' }}>No schedules found</p>
                            ...
                            return (
                                <div key={row.scheduleId} className="compliance-row">
                                    <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 120px 100px', gap: '8px', alignItems: 'center' }}>
                                        <div>
                                            <span className="badge" style={{ background: wc.bg, color: wc.color, marginBottom: '4px' }}>{row.wasteType.replace(/_/g, ' ')}</span>
                                            ...
                                            {row.collectionTime && (
                                                <p style={{ fontSize: '11px', color: '#94a3b8', margin: 0, display: 'flex', alignItems: 'center', gap: '3px' }}>
                                                    <span className="material-symbols-outlined" style={{ fontSize: '12px' }}>schedule</span>{row.collectionTime}
                                                </p>
                                            )}
                                        </div>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                            ...
                                            {row.wards.length > 2 && <span style={{ fontSize: '10px', color: '#94a3b8' }}>+{row.wards.length - 2}</span>}
                                        </div>
                                        <div>
                                            {row.status === 'upcoming' ? <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0 }}>—</p> : (
                                                <>
                                                    <p style={{ fontSize: '13px', fontWeight: 700, color: '#181c22', margin: '0 0 2px', fontFamily: 'Manrope,sans-serif' }}>{row.collected}/{row.scheduled}</p>
                                                    {row.skipped > 0 && <p style={{ fontSize: '10px', color: '#dc2626', margin: 0 }}>{row.skipped} skipped</p>}
                                                </>
                                            )}
                                        </div>
                                        <div>
                                            {row.status === 'upcoming' ? <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0 }}>Scheduled</p>
                                                : row.scheduled === 0 ? <p style={{ fontSize: '12px', color: '#dc2626', margin: 0 }}>No routes</p>
                                                    : <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 800, fontSize: '18px', color: pColor, margin: 0, lineHeight: 1 }}>{row.compliancePct}%</p>}
                                        </div>
                                        <div>
                                            {row.status !== 'upcoming' && row.scheduled > 0 ? (
                                                <div className="progress-track"><div className="progress-fill" style={{ width: `${row.compliancePct}%`, background: pColor }} /></div>
                                            ) : (
                                                <div className="progress-track"><div style={{ height: '100%', background: '#e2e8f0', borderRadius: '99px', width: '100%' }} /></div>
                                            )}
                                        </div>
                                        <div>
                ...
                <div style={{ padding: '12px 24px', borderTop: '1px solid rgba(0,69,13,0.06)', background: '#f9fdf9', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#717a6d', margin: 0 }}>
                        Compliant = ≥90% stops collected · Partial = at least 1 stop collected · Missed = no collections recorded
                    </p>
                </div>
            </div>
        </DashboardLayout>
