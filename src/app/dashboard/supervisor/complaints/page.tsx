...
interface Complaint {
    id: string; complaint_type: string; description: string; district: string
    status: string; created_at: string; resolution_notes: string | null
    reporter_name?: string; custom_complaint_type?: string
}
...
export default function SupervisorComplaintsPage() {
    ...
    async function loadData() {
        ...
        setComplaints((data || []).map((c: any) => ({ ...c, reporter_name: c.profiles?.full_name || 'Unknown Resident' })))
        setLoading(false)
    }
    ...
    async function updateStatus(complaint: Complaint, status: string) {
        setUpdating(true)
        const supabase = createClient()
        await supabase.from('complaints').update({ status, resolution_notes: resolutionNotes || null }).eq('id', complaint.id)
    ...
    return (
        ...
        .complaint-row { padding:18px 24px; border-bottom:1px solid rgba(0,69,13,0.04); transition:background 0.2s; display:flex; align-items:flex-start; gap:16px; cursor:pointer; }
        ...
        .status-badge { display:inline-flex; align-items:center; gap:4px; padding:3px 10px; border-radius:99px; font-size:10px; font-weight:700; font-family:'Manrope',sans-serif; letter-spacing:0.08em; text-transform:uppercase; }
      ...
      `}</style>
                ...
                <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
                    <div style={{ background: 'white', borderRadius: '20px', padding: '28px', width: '100%', maxWidth: '520px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '10px', flexShrink: 0, background: (COMPLAINT_TYPES[selected.complaint_type] || COMPLAINT_TYPES.other).bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <span className="material-symbols-outlined" style={{ fontSize: '20px', color: (COMPLAINT_TYPES[selected.complaint_type] || COMPLAINT_TYPES.other).color }}>{(COMPLAINT_TYPES[selected.complaint_type] || COMPLAINT_TYPES.other).icon}</span>
                                </div>
                                <div>
                                    <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '14px', color: '#181c22', margin: 0 }}>{(COMPLAINT_TYPES[selected.complaint_type] || COMPLAINT_TYPES.other).label}</p>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button className="action-btn" onClick={() => updateStatus(selected, 'in_progress')} disabled={updating} style={{ background: '#eff6ff', color: '#1d4ed8' }}>
                                <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>pending</span>In Progress
                            </button>
                            <button className="action-btn" onClick={() => updateStatus(selected, 'resolved')} disabled={updating} style={{ background: '#f0fdf4', color: '#00450d' }}>
                                <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>check_circle</span>Resolved
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <section style={{ marginBottom: '32px' }} className="s1">
                ...
                <p style={{ fontSize: '13px', color: '#717a6d', margin: '6px 0 0' }}>
                    Complaints submitted by residents in your district
                </p>
            </section>
            ...
            <div className="bento-card s3">
                <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '16px', color: '#181c22', margin: 0 }}>Complaint Feed</h3>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '64px 24px' }}>
                        ...
                        <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: '15px', color: '#181c22', margin: '0 0 6px' }}>No complaints found</p>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                            <p style={{ fontSize: '14px', fontWeight: 700, color: '#181c22', fontFamily: 'Manrope,sans-serif', margin: 0 }}>
                                                {ct.label}{complaint.custom_complaint_type && ` — ${complaint.custom_complaint_type}`}
                                            </p>
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '12px', color: '#94a3b8' }}>
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                                                <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>person</span>{complaint.reporter_name}
                                            </span>
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                                        ...
                                        {complaint.resolution_notes && (
                                            <p style={{ fontSize: '12px', color: '#717a6d', marginTop: '6px', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>sticky_note_2</span>{complaint.resolution_notes}
                                            </p>
                                        )}
                                    </div>
                                    {canUpdate && (
                                        <div style={{ flexShrink: 0, padding: '6px 12px', borderRadius: '8px', background: '#f0fdf4', color: '#00450d', fontSize: '12px', fontWeight: 700, fontFamily: 'Manrope,sans-serif', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                            <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>edit</span>Update
                                        </div>
                                    )}
                                </div>
                            )
