...
const MATERIAL_TYPES = ['Plastic', 'Paper', 'Glass', 'Metal', 'E-Waste', 'Organic', 'Mixed', 'Other']
...
function RecyclerLogPageContent() {
    ...
    const [formData, setFormData] = useState<FormData>({
        ...
        condition: '', notes: '', is_rejected: false, rejection_reason: '', rejection_notes: '',
    })
    ...
    async function autoVerifyCode(handoffCode: string) {
        ...
        try {
            const res = await fetch('/api/handoff/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: handoffCode }) })
            const data = await res.json()
            if (!res.ok) {
                const supabase = createClient()
                const { data: h } = await supabase.from('route_handoffs')
            ...
            } else { setHandoff(data.handoff); setTotalStops(data.totalStops); setCompletedStops(data.completedStops) }
    ...
    async function verifyCode() {
        ...
        try {
            const res = await fetch('/api/handoff/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: fullCode }) })
            const data = await res.json()
    ...
    function calculateAmount() {
        if (!formData.actual_quantity || !formData.material_type) return '0'
    ...
    async function handleSubmit() {
        ...
        if (!formData.material_type && !formData.is_rejected) { setSubmitError('Please select the material type'); return }
        if (!formData.grade && !formData.is_rejected) { setSubmitError('Please select the material grade'); return }
        if (formData.is_rejected && !formData.rejection_reason) { setSubmitError('Please select a rejection reason'); return }
        setSubmitting(true); setSubmitError('')
        const supabase = createClient()
        const { data: { user } } = await supabase.auth.getUser()
        const rate = getRateForMaterial(formData.material_type)
        try {
            const res = await fetch('/api/intake/create', {
                ...
                body: JSON.stringify({
                    ...
                    operator_id: user?.id,
                    operator_type: 'recycling_partner',
    ...
    return (
        ...
        .select-field:focus{border-color:#15803d;background-color:white;box-shadow:0 0 0 3px rgba(21,128,61,0.08);}
        ...
        .condition-card:hover{border-color:#15803d;background:#f0fdf4;}
        .condition-card.selected{border-color:#15803d;background:#f0fdf4;}
        .submit-btn{background:#15803d;color:white;border:none;border-radius:12px;padding:16px;width:100%;font-family:'Manrope',sans-serif;font-weight:700;font-size:15px;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;}
        .submit-btn:hover{background:#166534;box-shadow:0 8px 24px rgba(21,128,61,0.25);}
        ...
        .code-input:focus{border-color:#15803d;background:white;box-shadow:0 0 0 3px rgba(21,128,61,0.08);}
        .code-input.filled{border-color:#1b5e20;background:#f0fdf4;color:#15803d;}
        ...
        .tx-hash{font-family:'Courier New',monospace;font-size:11px;color:#15803d;word-break:break-all;line-height:1.5;}
        @keyframes spin{to{transform:rotate(360deg)}}
        @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}
        .fade-1{animation:fadeUp 0.4s ease 0.05s both}.fade-2{animation:fadeUp 0.4s ease 0.1s both}
      `}</style>

            {/* Header */}
            <section style={{ marginBottom: 32 }} className="fade-1">
                <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.18em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>
                    Recycling Partner · Waste Intake
                </p>
                <h1 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 40, fontWeight: 900, color: '#181c22', lineHeight: 1.1, margin: 0 }}>
                    {step === 'success' ? 'Intake Confirmed ✅' : <>Log Waste <span style={{ color: '#15803d' }}>Intake</span></>}
                </h1>
            </section>
            ...
            <div style={{ maxWidth: 640 }}>
                ...
                {loadingHandoff && (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, background: 'white', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ width: 28, height: 28, border: '2px solid #15803d', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .8s linear infinite', margin: '0 auto 12px' }} />
                    ...
                    <div className="fade-2">
                        {!showCodeEntry ? (
                            <div style={{ background: 'white', borderRadius: 20, padding: 40, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,69,13,0.04)', textAlign: 'center' }}>
                                <div style={{ width: 64, height: 64, background: '#f0fdf4', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                                    <span className="msf" style={{ color: '#15803d', fontSize: 32 }}>info</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: 12 }}>
                                    <button onClick={() => router.push('/dashboard/recycling-partner')} style={{ flex: 1, padding: 14, borderRadius: 12, border: '1.5px solid #e5e7eb', background: 'white', fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, cursor: 'pointer', color: '#181c22' }}>← Back</button>
                                    <button onClick={() => setShowCodeEntry(true)} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#15803d', color: 'white', fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, cursor: 'pointer', border: 'none' }}>Enter Code Manually</button>
                                </div>
                            </div>
                        ) : (
                            <div style={{ background: 'white', borderRadius: 20, padding: 40, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,69,13,0.04)' }}>
                                <div style={{ textAlign: 'center', marginBottom: 32 }}>
                                    <div style={{ width: 64, height: 64, background: '#f0fdf4', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                                        <span className="msf" style={{ color: '#15803d', fontSize: 32 }}>pin</span>
                                    </div>
                ...
                {!loadingHandoff && handoff && step === 'form' && (
                    <div className="fade-2">
                        {/* Route summary */}
                        <div style={{ background: '#f0fdf4', border: '1px solid rgba(0,69,13,0.12)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, color: '#15803d', fontSize: 15, margin: 0 }}>✅ Route Details</p>
                                <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', padding: '2px 8px', borderRadius: 99, background: 'rgba(21,128,61,0.08)', color: '#15803d', fontFamily: 'Manrope,sans-serif' }}>Auto-filled</span>
                            </div>
                        ...
                        <div style={{ background: 'white', borderRadius: 16, padding: 32, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,69,13,0.04)', marginBottom: 16 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope,sans-serif', marginBottom: 8 }}>Material Type *</label>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#374151', fontFamily: 'Manrope,sans-serif', marginBottom: 8 }}>Material Grade *</label>
                                ...
                                {formData.material_type && formData.actual_quantity && (
                                    <div style={{ padding: '10px 14px', background: '#f0fdf4', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <span style={{ fontSize: 12, color: '#41493e' }}>Rate: LKR {getRateForMaterial(formData.material_type)}/kg</span>
                                        <span style={{ fontSize: 14, fontWeight: 700, color: '#15803d', fontFamily: 'Manrope,sans-serif' }}>Total: LKR {calculateAmount()}</span>
                                    </div>
                                )}
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {WASTE_CONDITIONS.map(c => (
                                            <div key={c.value} onClick={() => setFormData({ ...formData, condition: c.value })}
                                                className={`condition-card ${formData.condition === c.value ? 'selected' : ''}`}>{c.label}</div>
                                        ))}
                                    </div>
                                </div>
                ...
                {step === 'success' && completedIntake && (
                    <div className="fade-2">
                        <div style={{ background: 'white', borderRadius: 20, padding: 48, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,69,13,0.04)', textAlign: 'center' }}>
                            <div style={{ width: 72, height: 72, background: completedIntake.is_rejected ? '#fef2f2' : '#f0fdf4', borderRadius: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
                                <span className="msf" style={{ color: completedIntake.is_rejected ? '#ba1a1a' : '#15803d', fontSize: 40 }}>
                            ...
                            {!completedIntake.is_rejected && (
                                <div style={{ background: '#f0fdf4', borderRadius: 12, padding: 20, marginBottom: 24, textAlign: 'left' }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                        {[
                                            { label: 'Quantity', value: `${completedIntake.actual_quantity} ${completedIntake.unit}` },
                                            { label: 'Condition', value: completedIntake.condition },
                                            { label: 'Material', value: completedIntake.material_type || '—' },
                                            { label: 'Grade', value: completedIntake.grade?.replace('_', ' ').toUpperCase() || '—' },
                                        ].map(item => (
                                            <div key={item.label}>
                                    ...
                                    {completedIntake.total_amount && (
                                        <div style={{ borderTop: '1px solid rgba(0,69,13,0.1)', paddingTop: 14, marginTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <p style={{ fontSize: 13, color: '#41493e', margin: 0 }}>Payment due to CMC</p>
                                            <p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 900, fontSize: 20, color: '#15803d', margin: 0 }}>LKR {completedIntake.total_amount.toLocaleString()}</p>
                                        </div>
                                    )}
                                    {completedIntake.tx_hash && (
                                        <div style={{ borderTop: '1px solid rgba(0,69,13,0.1)', paddingTop: 14, marginTop: 14 }}>
                                            ...
                                            <div className="tx-box">
                                                <span className="msf" style={{ fontSize: 16, color: '#15803d', flexShrink: 0, marginTop: 1 }}>verified</span>
                                                <div>
                            ...
                            <div style={{ display: 'flex', gap: 12 }}>
                                <button onClick={() => router.push('/dashboard/recycling-partner')}
                                ...
                                <button onClick={() => router.push('/dashboard/recycling-partner/history')}
                                    style={{ flex: 1, padding: 14, borderRadius: 12, background: '#15803d', color: 'white', fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, cursor: 'pointer', border: 'none' }}>
                                    View History
                                </button>
                            </div>
                        </div>
...
export default function RecyclerLogPage() {
    return (
        <Suspense fallback={
