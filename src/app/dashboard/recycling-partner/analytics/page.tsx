...
function getMaterialStyle(mat: string) { return MATERIAL_COLORS[mat?.toLowerCase()] || MATERIAL_COLORS.other }

interface MonthlyData { month: string; label: string; totalWeight: number; totalAmount: number; intakeCount: number; rejectionCount: number }
interface MaterialBreakdown { material: string; totalWeight: number; totalAmount: number; count: number }
...
export default function RecyclerAnalyticsPage() {
    ...
    async function loadData() {
        ...
        setProfile(p)
        const { data } = await supabase.from('waste_intake_logs').select('*').eq('operator_id', user.id).eq('operator_type', 'recycling_partner').order('received_at', { ascending: false })
        const allLogs = data || []
        const logs = allLogs.filter((l: any) => !l.is_rejected)
        setIntakes(logs)
        ...
        allLogs.forEach((log: any) => {
            ...
            if (!monthMap[key]) monthMap[key] = { month: key, label, totalWeight: 0, totalAmount: 0, intakeCount: 0, rejectionCount: 0 }
            if (log.is_rejected) monthMap[key].rejectionCount++
            else { monthMap[key].totalWeight += log.actual_quantity || 0; monthMap[key].totalAmount += log.total_amount || 0; monthMap[key].intakeCount++ }
        })
        setMonthly(Object.values(monthMap).sort((a, b) => b.month.localeCompare(a.month)))
    ...
    function buildMaterials(logs: any[], month: string) {
        ...
        filtered.forEach((log: any) => {
            ...
            if (!matMap[mat]) matMap[mat] = { material: mat, totalWeight: 0, totalAmount: 0, count: 0 }
    ...
    const totalRejections = selectedMonth === 'all' ? monthly.reduce((s, m) => s + m.rejectionCount, 0) : (monthly.find(m => m.month === selectedMonth)?.rejectionCount || 0)
    ...
    return (
      ...
      `}</style>
            ...
            <div className="a1" style={{ marginBottom: 24 }}>
                <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.18em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', textTransform: 'uppercase', marginBottom: 6 }}>Recycling Partner · Analytics</p>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    <h1 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 40, fontWeight: 800, color: '#181c22', lineHeight: 1.05, margin: 0 }}>
                        Material <span style={{ color: '#15803d' }}>Analytics</span>
                    </h1>
                    <select value={selectedMonth} onChange={e => handleMonthChange(e.target.value)}
                        style={{ padding: '8px 14px', border: '1.5px solid #e5e7eb', borderRadius: 10, fontSize: 13, fontFamily: 'Inter,sans-serif', color: '#181c22', background: 'white', outline: 'none', cursor: 'pointer' }}>
            ...
            {loading ? (
                ...
                <>
                    <div className="a2" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
                        {[
                            { label: 'Volume Received', value: `${currentData.totalWeight.toFixed(1)} kg`, icon: 'scale', color: '#15803d', bg: '#f0fdf4' },
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                        <div className="a3 card" style={{ padding: '20px 22px' }}>
                            ...
                            {monthly.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><span className="msf" style={{ fontSize: 28, color: '#d1d5db', display: 'block', marginBottom: 8 }}>bar_chart</span><p style={{ fontSize: 12, color: '#94a3b8' }}>No data yet</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {monthly.slice(0, 6).map(m => {
                                        ...
                                        return (
                                            <div key={m.month} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => handleMonthChange(isSel ? 'all' : m.month)}>
                                                <span style={{ fontSize: 11, color: '#64748b', width: 56, flexShrink: 0, fontFamily: 'Manrope,sans-serif', fontWeight: isSel ? 700 : 400 }}>{m.label}</span>
                                                <div style={{ flex: 1, height: 22, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                                                    <div style={{ height: '100%', width: `${barW}%`, background: isSel ? '#15803d' : '#86efac', borderRadius: 6, transition: 'width 0.4s ease', minWidth: barW > 0 ? 4 : 0 }} />
                                                </div>
                                                <span style={{ fontSize: 11, fontWeight: 700, color: isSel ? '#15803d' : '#64748b', fontFamily: 'Manrope,sans-serif', width: 52, textAlign: 'right', flexShrink: 0 }}>{m.totalWeight.toFixed(0)}kg</span>
                                            </div>
                                        )
                                    })}
                                </div>
                        ...
                        <div className="a3 card" style={{ padding: '20px 22px' }}>
                            <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, color: '#181c22', margin: '0 0 16px' }}>Monthly Revenue (LKR)</h3>
                            {monthly.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><span className="msf" style={{ fontSize: 28, color: '#d1d5db', display: 'block', marginBottom: 8 }}>payments</span><p style={{ fontSize: 12, color: '#94a3b8' }}>No revenue data yet</p></div>
                            ) : (() => {
                                const maxAmt = Math.max(...monthly.map(m => m.totalAmount), 1)
                                return (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {monthly.slice(0, 6).map(m => {
                                            const barW = Math.round((m.totalAmount / maxAmt) * 100)
                                            const isSel = selectedMonth === m.month
                                            return (
                                                <div key={m.month} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => handleMonthChange(isSel ? 'all' : m.month)}>
                                                    <span style={{ fontSize: 11, color: '#64748b', width: 56, flexShrink: 0, fontFamily: 'Manrope,sans-serif', fontWeight: isSel ? 700 : 400 }}>{m.label}</span>
                                                    <div style={{ flex: 1, height: 22, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                                                        <div style={{ height: '100%', width: `${barW}%`, background: isSel ? '#7c3aed' : '#c4b5fd', borderRadius: 6, transition: 'width 0.4s ease', minWidth: barW > 0 ? 4 : 0 }} />
                                                    </div>
                                                    <span style={{ fontSize: 11, fontWeight: 700, color: isSel ? '#7c3aed' : '#64748b', fontFamily: 'Manrope,sans-serif', width: 64, textAlign: 'right', flexShrink: 0 }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                                                    <span style={{ fontSize: 12, fontWeight: 700, color: '#15803d', fontFamily: 'Manrope,sans-serif' }}>{mat.totalWeight.toFixed(1)} kg</span>
                                                    {mat.totalAmount > 0 && <span style={{ fontSize: 12, fontWeight: 700, color: '#7c3aed', fontFamily: 'Manrope,sans-serif' }}>LKR {Math.round(mat.totalAmount).toLocaleString()}</span>}
                                                    <span style={{ fontSize: 11, color: '#94a3b8', width: 36, textAlign: 'right' }}>{share.toFixed(0)}%</span>
                                                </div>
                                            </div>
                                            <div style={{ height: 6, background: '#f1f5f9', borderRadius: 99, overflow: 'hidden' }}>
                                                <div style={{ height: '100%', width: `${share}%`, background: style.color, borderRadius: 99, opacity: 0.7, transition: 'width 0.5s ease' }} />
                                            </div>
                                        </div>
                                    </div>
                                )
                        ...
                        {materials.length > 0 && (
                            <div style={{ padding: '12px 18px', background: '#f9fbf9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <div style={{ display: 'flex', gap: 20 }}>
                                    <span style={{ fontSize: 13, fontWeight: 700, color: '#15803d', fontFamily: 'Manrope,sans-serif' }}>{materials.reduce((s, m) => s + m.totalWeight, 0).toFixed(1)} kg</span>
                                    <span style={{ fontSize: 13, fontWeight: 700, color: '#7c3aed', fontFamily: 'Manrope,sans-serif' }}>LKR {Math.round(materials.reduce((s, m) => s + m.totalAmount, 0)).toLocaleString()}</span>
                                </div>
                            </div>
                        )}
                    </div>
