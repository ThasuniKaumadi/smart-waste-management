...
export default function DESchedulesPage() {
  ...
  async function loadData() {
    ...
    // Get wards for this DE's district
    if (p?.district) {
    ...
    >
      <style>{`
        ...
        .schedule-row {
          ...
          transition: background 0.2s ease; display: flex; align-items: center; gap: 16px;
        }
        ...
        {[
          ...
          <div key={m.label} className="bento-card p-5">
            ...
            <p className="text-xs font-bold uppercase"
              style={{ letterSpacing: '0.1em', color: '#94a3b8', fontFamily: 'Manrope, sans-serif' }}>{m.label}</p>
          </div>
        ))}
      </div>
      ...
      {message && (
        <div className="mb-6 flex items-center gap-3 p-4 rounded-xl text-sm s2"
          style={{
            ...
            border: `1px solid ${message.startsWith('Error') ? 'rgba(186,26,26,0.2)' : 'rgba(0,69,13,0.15)'}`,
            color: message.startsWith('Error') ? '#ba1a1a' : '#00450d',
          }}>
          <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>
      ...
      {showForm && (
        <div className="bento-card mb-8 s2 slide-down">
          <div className="px-8 py-6" style={{ borderBottom: '1px solid rgba(0,69,13,0.06)' }}>
            ...
            <p className="text-sm mt-1" style={{ color: '#717a6d' }}>
              This schedule will be visible to all residents in{' '}
              <span style={{ fontWeight: 700, color: '#00450d' }}>{profile?.district}</span>
              {formData.ward && <> — Ward: <span style={{ fontWeight: 700, color: '#00450d' }}>{formData.ward}</span></>}
            </p>
          </div>
          <form onSubmit={handleSubmit} className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">

              {/* Ward — optional, scopes to specific ward */}
              <div>
                <label className="field-label">
                  Ward (Optional — leave blank for entire district)
                </label>
                <select className="select-field" value={formData.ward}
                  onChange={e => setFormData({ ...formData, ward: e.target.value })}>
                  <option value="">All wards in {profile?.district}</option>
              ...
              <div className="md:col-span-2">
                ...
                <input type="text" className="form-field"
                  placeholder="Any additional information for residents e.g. Please separate organic from inorganic"
                  value={formData.notes}
                  onChange={e => setFormData({ ...formData, notes: e.target.value })} />
              </div>
            </div>

            {/* District locked notice */}
            <div className="flex items-center gap-3 mt-5 p-4 rounded-xl"
              ...
              <p className="text-sm" style={{ color: '#41493e' }}>
                This schedule will be automatically assigned to{' '}
                <strong style={{ color: '#00450d' }}>{profile?.district}</strong>.
                Only residents registered in this district will see it.
              </p>
            </div>
      ...
      <div className="bento-card s3">
        <div className="px-6 py-5 flex flex-wrap items-center justify-between gap-3"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              style={{ appearance: 'none', paddingRight: '12px', background: filterWasteType !== 'all' ? '#00450d' : '#f1f5f9', color: filterWasteType !== 'all' ? 'white' : '#64748b', border: 'none', cursor: 'pointer' }}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-8">
            ...
            <p className="text-sm mb-6" style={{ color: '#94a3b8' }}>
              Create your first collection schedule for {profile?.district}
            </p>
              ...
              return (
                <div key={schedule.id} className="schedule-row">
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: waste.bg }}>
                    <span className="material-symbols-outlined"
                      style={{ color: waste.color, fontSize: '18px' }}>delete_sweep</span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '2px 8px', borderRadius: '99px', background: waste.bg, color: waste.color, fontFamily: 'Manrope, sans-serif' }}>
                        {schedule.waste_type}
                      </span>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className={`action-btn ${schedule.published ? 'unpublish-btn' : 'publish-btn'}`}>
        ...
        <div className="px-8 py-4 flex items-center gap-3"
          ...
          <p className="text-xs" style={{ color: '#717a6d' }}>
            Published schedules are visible to all residents registered in <strong>{profile?.district}</strong>.
            Draft schedules are only visible to you.
          </p>
        </div>
      </div>
    </DashboardLayout>
