...
export default function DESchedulesPage() {
  ...
  async function loadData() {
    ...
    if (p?.district) {
      ...
      // Load supervisors in this district
      const { data: supData } = await supabase
        .from('profiles')
        .select('id, full_name, district, assigned_wards')
        .eq('role', 'supervisor')
  ...
  async function togglePublish(schedule: Schedule) {
    const supabase = createClient()
    await supabase.from('schedules').update({ published: !schedule.published, status: !schedule.published ? 'published' : 'draft' }).eq('id', schedule.id)
    loadData()
  }
    ...
    >
      ...
      <section className="mb-10 s1">
        <span className="text-xs font-bold uppercase block mb-2" style={{ letterSpacing: '0.2em', color: '#717a6d', fontFamily: 'Manrope, sans-serif' }}>
          District Engineering · Schedule Management
        </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                <label className="field-label">
                  Wards <span style={{ color: '#94a3b8', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional — leave blank for entire district)</span>
                </label>
              ...
              <div className="md:col-span-2">
                <label className="field-label">
                  Assign Supervisor <span style={{ color: '#94a3b8', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span>
                </label>
                {supervisors.length === 0 ? (
                  <div style={{ padding: '12px 14px', borderRadius: '10px', background: '#fefce8', border: '1px solid rgba(217,119,6,0.2)', fontSize: '13px', color: '#92400e', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    No supervisors found in {profile?.district}. Assign a supervisor to this district first.
                  </div>
                ) : (
                  <select className="select-field" value={formData.supervisor_id}
                    onChange={e => setFormData({ ...formData, supervisor_id: e.target.value })}>
                    <option value="">— No supervisor assigned —</option>
                    {supervisors.map(s => (
                      <option key={s.id} value={s.id}>
                        {s.full_name}{s.assigned_wards?.length > 0 ? ` (${s.assigned_wards.join(', ')})` : ' (all wards)'}
                      </option>
                    ))}
                  </select>
                )}
              ...
              <div>
                ...
                <select className="select-field" value={formData.waste_type}
                  onChange={e => setFormData({ ...formData, waste_type: e.target.value })} required>
              ...
              <div>
                ...
                <input type="date" className="form-field" value={formData.scheduled_date}
                  min={new Date().toISOString().split('T')[0]}
                  onChange={e => setFormData({ ...formData, scheduled_date: e.target.value })} required />
              </div>

              {/* Collection day */}
              <div>
                <label className="field-label">Collection Day *</label>
                <select className="select-field" value={formData.collection_day}
                  onChange={e => setFormData({ ...formData, collection_day: e.target.value })} required>
              ...
              <div>
                ...
                <input type="time" className="form-field" value={formData.collection_time}
                  onChange={e => setFormData({ ...formData, collection_time: e.target.value })} required />
              </div>

              {/* Frequency */}
              <div>
                <label className="field-label">Frequency *</label>
                <select className="select-field" value={formData.frequency}
              ...
              <div className="md:col-span-2">
                ...
                <input type="text" className="form-field"
                  placeholder="Any additional information e.g. Please separate organic from inorganic"
                  value={formData.notes}
                  onChange={e => setFormData({ ...formData, notes: e.target.value })} />
              </div>
            </div>

            <div className="flex items-center gap-3 p-4 rounded-xl mt-5"
              ...
              <p className="text-sm" style={{ color: '#41493e' }}>
                {selectedWards.length > 0
                  ? <>Visible to residents in wards: <strong style={{ color: '#00450d' }}>{selectedWards.join(', ')}</strong></>
                  : <>Visible to <strong style={{ color: '#00450d' }}>all residents</strong> in {profile?.district}</>
                }
                {formData.supervisor_id && supervisors.find(s => s.id === formData.supervisor_id) && (
                  <> · Supervisor: <strong style={{ color: '#00450d' }}>{supervisors.find(s => s.id === formData.supervisor_id)?.full_name}</strong></>
                )}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 mt-5">
              <button type="submit" disabled={saving} className="submit-btn">
                {saving ? (
                  ...
                  <><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>add_circle</span>Create Schedule</>
                )}
              </button>
      ...
      <div className="bento-card s3">
        <div className="px-6 py-5 flex flex-wrap items-center justify-between gap-3" style={{ borderBottom: '1px solid rgba(0,69,13,0.06)' }}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select className="filter-btn" style={{ appearance: 'none', paddingRight: '12px', background: filterWasteType !== 'all' ? '#00450d' : '#f1f5f9', color: filterWasteType !== 'all' ? 'white' : '#64748b', border: 'none', cursor: 'pointer' }}
              value={filterWasteType} onChange={e => setFilterWasteType(e.target.value)}>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-8">
            ...
            <button onClick={() => setShowForm(true)} className="submit-btn" style={{ width: 'auto' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>add</span>New Schedule
            </button>
          </div>
        ) : (
          <div>
                ...
                <div key={schedule.id} className="schedule-row">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {supName && (
                        <span style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '2px 8px', borderRadius: '99px', background: 'rgba(124,58,237,0.08)', color: '#7c3aed', fontFamily: 'Manrope, sans-serif' }}>
                          {supName}
                        </span>
                      )}
                    </div>
        ...
        <div className="px-8 py-4 flex items-center gap-3" style={{ borderTop: '1px solid rgba(0,69,13,0.06)', background: '#f9f9ff' }}>
          ...
          <p className="text-xs" style={{ color: '#717a6d' }}>
            Published schedules are visible to residents based on their registered ward in <strong>{profile?.district}</strong>.
          </p>
        </div>
      </div>
    </DashboardLayout>
