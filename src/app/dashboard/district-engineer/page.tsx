...
export default function DistrictEngineerDashboardPage() {
  ...
  async function loadData() {
    ...
    const { data: rData } = await supabase
      .from('routes').select('status').eq('district', district)
    if (rData) {
      const counts: Record<string, number> = {}
      rData.forEach(r => { if (r.status) counts[r.status] = (counts[r.status] || 0) + 1 })
      setRouteData(Object.entries(counts).map(([name, value]) => ({ name, value })))
    }
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm mt-1" style={{ color: '#717a6d' }}>
                    Distribution by type in {profile?.district || 'your district'}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  {recentComplaints.map(c => (
                    <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl"
                      ...
                      <span className="status-badge" style={statusStyle(c.status)}>
                        {c.status?.replace('_', ' ')}
                      </span>
                    </div>
                  ))}
