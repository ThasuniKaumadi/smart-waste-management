...
export default function ContractorBreakdownsPage() {
  ...
  async function loadData() {
    ...
    const { data, error } = await supabase
      ...
      .select('*, profiles!breakdown_reports_driver_id_fkey(full_name)')
    ...
    if (error) {
      const { data: fallback } = await supabase
        ...
        .in('driver_id', driverIds)
        .order('created_at', { ascending: false })
      setBreakdowns((fallback || []).map((r: any) => ({ ...r, driver_name: 'Driver' })))
    } else {
  ...
  return (
    <DashboardLayout role="Contractor" userName={profile?.full_name || profile?.organisation_name} navItems={CONTRACTOR_NAV}>
      <style>{`
        ...
        .filter-pill { border:1.5px solid #e4ede4; border-radius:9999px; padding:7px 16px; font-family:'Manrope',sans-serif; font-weight:600; font-size:12px; cursor:pointer; transition:all 0.18s; color:#717a6d; background:white; display:flex; align-items:center; gap:6px; }
        ...
        .bcard { background:white; border-radius:16px; border:1px solid rgba(0,0,0,0.06); box-shadow:0 1px 4px rgba(0,0,0,0.04); transition:all 0.18s; overflow:hidden; }
        .bcard:hover { box-shadow:0 4px 16px rgba(0,0,0,0.08); transform:translateY(-1px); }
        .bcard.urgent { border-left:3px solid #dc2626; }
        .action-btn { border:none; border-radius:9px; padding:9px 16px; font-family:'Manrope',sans-serif; font-weight:700; font-size:12px; cursor:pointer; transition:all 0.18s; display:flex; align-items:center; gap:6px; }
        .action-btn.dispatch { background:rgba(180,83,9,0.09); color:#b45309; border:1.5px solid rgba(180,83,9,0.2); }
        .action-btn.dispatch:hover { background:rgba(180,83,9,0.15); }
        .action-btn.resolve { background:rgba(0,69,13,0.08); color:#00450d; border:1.5px solid rgba(0,69,13,0.2); }
        .action-btn.resolve:hover { background:rgba(0,69,13,0.13); }
        .action-btn:disabled { opacity:0.5; cursor:not-allowed; }
        .resolve-input { width:100%; border:1.5px solid #e4ede4; border-radius:9px; padding:10px 14px; font-size:13px; font-family:'Inter',sans-serif; outline:none; transition:border-color 0.2s; box-sizing:border-box; resize:none; min-height:72px; }
      ...
      {toast && (
        <div className="toast-anim" style={{ position:'fixed', bottom:'24px', left:'50%', background:toastType==='error'?'#dc2626':'#181c22', color:'white', padding:'10px 20px', borderRadius:'9999px', fontSize:'13px', fontWeight:500, zIndex:1000, display:'flex', alignItems:'center', gap:'8px', boxShadow:'0 4px 20px rgba(0,0,0,0.2)' }}>
          <span className="msf" style={{ fontSize:'16px', color:toastType==='error'?'#fca5a5':'#4ade80' }}>{toastType==='error'?'error':'check_circle'}</span>
          {toast}
        </div>
      )}
      ...
      <main style={{ maxWidth:'900px', margin:'0 auto', padding:'32px 24px' }}>
        ...
        <div className="a1" style={{ marginBottom:'28px' }}>
          <p style={{ fontSize:'11px', color:'#717a6d', fontWeight:600, letterSpacing:'0.18em', textTransform:'uppercase', margin:'0 0 6px' }}>🚨 FLEET MANAGEMENT</p>
          ...
          <p style={{ fontSize:'14px', color:'#717a6d', margin:0 }}>Breakdown reports submitted by your drivers</p>
        </div>
        ...
        <div className="a2" style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:'14px', marginBottom:'28px' }}>
          {[
            ...
            <div key={stat.label} style={{ background:'white', borderRadius:'16px', padding:'18px 20px', border:'1px solid rgba(0,0,0,0.06)', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        {urgentCount > 0 && (
          <div className="a2" style={{ background:'rgba(220,38,38,0.04)', border:'1px solid rgba(220,38,38,0.15)', borderRadius:'14px', padding:'14px 20px', marginBottom:'20px', display:'flex', alignItems:'center', gap:'12px' }}>
        ...
        <div className="a3" style={{ display:'flex', gap:'8px', marginBottom:'20px', flexWrap:'wrap' }}>
          {FILTERS.map(f => {
            ...
            return (
              <button key={f} onClick={() => setFilter(f)} className={`filter-pill ${isActive ? (isRed ? 'active-red' : 'active') : ''}`}>
                {f === 'all' ? 'All' : STATUS_CONFIG[f]?.label}
                <span style={{ background:isActive?'rgba(255,255,255,0.25)':'#f0f4f0', color:isActive?'white':'#717a6d', borderRadius:'9999px', padding:'1px 7px', fontSize:'11px', fontWeight:700 }}>{counts[f]}</span>
              </button>
            )
          })}
        </div>
              ...
              return (
                <div key={b.id} className={`bcard ${isUrgent ? 'urgent' : ''}`}>
                  <div style={{ padding:'18px 20px', display:'flex', alignItems:'flex-start', gap:'14px' }}>
                    <div style={{ width:'48px', height:'48px', borderRadius:'13px', flexShrink:0, background:isUrgent?'rgba(220,38,38,0.08)':'rgba(0,69,13,0.07)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <span className="msf" style={{ fontSize:'24px', color:isUrgent?'#dc2626':'#00450d' }}>{bt.icon}</span>
                    </div>

                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'5px', flexWrap:'wrap' }}>
                        <span style={{ fontSize:'15px', fontWeight:700, color:'#181c22', fontFamily:'Manrope, sans-serif' }}>{bt.label}</span>
                        <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 9px', borderRadius:'9999px', background:sc.bg, color:sc.color, textTransform:'uppercase', letterSpacing:'0.07em', display:'inline-flex', alignItems:'center', gap:'4px' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'16px', flexWrap:'wrap' }}>
                        <span style={{ fontSize:'12px', color:'#717a6d', display:'flex', alignItems:'center', gap:'3px' }}>
                          <span className="msf" style={{ fontSize:'13px' }}>person</span>{b.driver_name}
                        </span>
                        <span style={{ fontSize:'12px', color:'#717a6d', display:'flex', alignItems:'center', gap:'3px' }}>
                          <span className="msf" style={{ fontSize:'13px' }}>directions_car</span>{b.vehicle_number}
                        </span>
                        <span style={{ fontSize:'12px', color:'#717a6d', display:'flex', alignItems:'center', gap:'3px' }}>
                          <span className="msf" style={{ fontSize:'13px' }}>location_on</span>{b.location_address}
                        </span>
                      </div>
                      {b.description && (
                        <p style={{ fontSize:'12px', color:'#717a6d', margin:'5px 0 0', fontStyle:'italic' }}>"{b.description}"</p>
                      )}
                      {b.resolution_notes && b.status === 'resolved' && (
                        <p style={{ fontSize:'12px', color:'#15803d', margin:'5px 0 0', display:'flex', alignItems:'flex-start', gap:'4px' }}>
                          <span className="msf" style={{ fontSize:'13px', marginTop:'1px' }}>check_circle</span>{b.resolution_notes}
                        </p>
                      )}
                    </div>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
                      <div style={{ textAlign:'right' }}>
                        <p style={{ fontSize:'13px', fontWeight:600, color:'#41493e', margin:0 }}>{new Date(b.created_at).toLocaleDateString('en-GB', { day:'numeric', month:'short' })}</p>
                        <p style={{ fontSize:'11px', color:'#9ca3af', margin:0 }}>{new Date(b.created_at).toLocaleTimeString('en-GB', { hour:'2-digit', minute:'2-digit' })}</p>
                      </div>
                      {sc.next && (
                        sc.next === 'resolved' ? (
                          <button className="action-btn resolve" onClick={() => setExpandedId(isExpanded ? null : b.id)}>
                            <span className="msf" style={{ fontSize:'14px' }}>expand_more</span>Resolve
                          </button>
                        ) : (
                          <button className="action-btn dispatch" disabled={updating === b.id} onClick={() => updateStatus(b, sc.next!)}>
                            {updating === b.id ? (
                  ...
                  {isExpanded && (
                    <div style={{ borderTop:'1px solid rgba(0,69,13,0.08)', padding:'16px 20px', background:'rgba(0,69,13,0.02)' }}>
                      ...
                      <textarea className="resolve-input" placeholder="Describe how the breakdown was resolved..." value={resolutionNote} onChange={e => setResolutionNote(e.target.value)} />
                      <div style={{ display:'flex', gap:'10px', marginTop:'12px' }}>
                        <button className="action-btn resolve" style={{ padding:'10px 20px' }} disabled={updating === b.id} onClick={() => updateStatus(b, 'resolved')}>
                          {updating === b.id ? (
                            <svg style={{ width:'13px', height:'13px', animation:'spin 0.8s linear infinite' }} fill="none" viewBox="0 0 24 24">
                              <circle style={{ opacity:0.25 }} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path style={{ opacity:0.75 }} fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                        ...
                        <button style={{ background:'none', border:'none', fontSize:'13px', color:'#717a6d', cursor:'pointer', fontFamily:'Manrope, sans-serif', fontWeight:600 }} onClick={() => { setExpandedId(null); setResolutionNote('') }}>
                          Cancel
                        </button>
                      </div>
                    </div>
