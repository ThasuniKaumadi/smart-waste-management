...
export default function ContractorIncidentsPage() {
    ...
    async function createIncident() {
        if (!form.title || !form.description || !form.type) {
            setErrorMsg('Title, type and description are required.')
            return
        }
        setSubmitting(true)
        setErrorMsg('')
        ...
        if (error) {
            setErrorMsg('Failed to report incident: ' + error.message)
        } else {
            setSuccessMsg('Incident reported successfully. CMC has been notified.')
    ...
    const filteredIncidents = incidents.filter(i => {
        const tabMatch = activeTab === 'all' ? true
            : activeTab === 'open' ? !['resolved', 'closed'].includes(i.status)
    ...
    return (
        <DashboardLayout
            ...
            primaryAction={{ label: 'Report Incident', href: '#', icon: 'add' }}
        >
            <style>{`
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-2 text-sm" style={{ color: '#717a6d', fontFamily: 'Inter, sans-serif' }}>
                            Report and track field incidents and exceptions
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-4 s3">
                        {(['all', 'open', 'resolved'] as const).map(tab => (
                            <button key={tab} className={`tab-btn ${activeTab === tab ? 'tab-active' : 'tab-inactive'}`}
                                ...
                                {tab === 'all' ? `All (${incidents.length})` :
                                    tab === 'open' ? `Open (${stats.open})` :
                        ...
                        {filteredIncidents.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                <p className="text-sm mt-1 mb-6" style={{ color: '#94a3b8' }}>
                                    {activeTab === 'open' ? 'No open incidents — great work!' : 'No incidents match your filters'}
                                </p>
                                    ...
                                    <div key={inc.id} className="incident-row" onClick={() => loadIncidentDetails(inc)}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs" style={{ color: '#717a6d' }}>
                                                {incidentTypeLabel(inc.type)} · {inc.ward || inc.district || 'Unknown location'} · {new Date(inc.created_at).toLocaleDateString('en-GB')}
                                            </p>
                                        </div>
                            ...
                            <div className="w-full max-w-2xl bento-card p-8" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
                                ...
                                {selectedIncident.resolution_notes && (
                                    <div className="p-4 rounded-xl mb-6" style={{ background: '#f0fdf4', border: '1px solid rgba(0,69,13,0.1)' }}>
                                        <p className="text-xs font-bold uppercase mb-2"
                                            style={{ color: '#00450d', letterSpacing: '0.08em', fontFamily: 'Manrope, sans-serif' }}>
                                            Resolution Notes
                                        </p>
                                        <p className="text-sm" style={{ color: '#00450d', fontFamily: 'Inter, sans-serif' }}>
                                            {selectedIncident.resolution_notes}
                                        </p>
                                    </div>
                                )}
                                ...
                                {responses.length > 0 && (
                                    <div className="mb-6">
                                        <p className="text-xs font-bold uppercase mb-3"
                                            style={{ color: '#94a3b8', letterSpacing: '0.1em', fontFamily: 'Manrope, sans-serif' }}>
                                            CMC Responses ({responses.length})
                                        </p>
                                        {responses.map(r => (
                                            <div key={r.id} className="response-item">
                                                ...
                                                <p className="text-xs mt-2" style={{ color: '#94a3b8' }}>
                                                    {new Date(r.responded_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex gap-3">
                                    {!['resolved', 'closed'].includes(selectedIncident.status) && (
                                        <button className="btn-secondary flex-1 justify-center"
                                            onClick={() => closeIncident(selectedIncident.id)}>
                            ...
                            <div className="w-full max-w-2xl bento-card p-8" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p className="text-xs mt-1" style={{ color: '#717a6d' }}>
                                            Report a field incident or exception to CMC
                                        </p>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input className="form-input" placeholder="Brief description of the incident"
                                            value={form.title}
                                    ...
                                    <div>
                                        ...
                                        <input className="form-input" placeholder="e.g. 45 Main Street, Ward 1, Colombo"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input className="form-input" placeholder="e.g. Colombo"
                                                value={form.district}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <select className="form-input"
                                                ...
                                                <option value="">None</option>
                                                {routes.map(r => (
                                                    <option key={r.id} value={r.id}>{r.route_name || `${r.ward} - ${r.district}`}</option>
                                                ))}
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div className="flex gap-3 mt-6">
                                    ...
                                    <button className="btn-primary flex-1 justify-center"
                                        onClick={createIncident} disabled={submitting}>
                                        {submitting ? (
