...
const COMMERCIAL_NAV = [
    ...
    { label: 'Report Issue', href: '/dashboard/commercial/report', icon: 'report_problem' },
...
const CATEGORIES = [
    {
        ...
        items: [
            ...
            { value: 'billing_issue', label: 'Billing Issue', icon: 'receipt_long', color: '#0891b2', bg: '#ecfeff', table: 'complaints' },
...
interface HistoryItem {
    id: string; type: 'complaint' | 'waste_report'
    category: string; custom_category?: string
    description: string; status: string; created_at: string
    location_address?: string; photo_url?: string; blockchain_tx?: string
}
...
export default function CommercialReportPage() {
    ...
    async function loadData() {
        ...
        const complaints: HistoryItem[] = (complaintsRes.data || []).map((c: any) => ({
            id: c.id, type: 'complaint', category: c.complaint_type || 'other',
            custom_category: c.custom_complaint_type, description: c.description,
            status: c.status, created_at: c.created_at, blockchain_tx: c.blockchain_tx,
        }))
        const reports: HistoryItem[] = (reportsRes.data || []).map((r: any) => ({
            id: r.id, type: 'waste_report', category: r.report_type || 'other',
            description: r.description, status: r.status, created_at: r.created_at,
            location_address: r.location_address, photo_url: r.photo_url,
        }))
        setHistory([...complaints, ...reports].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()))
        setLoading(false)
    }
    ...
    function getLocation() {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            () => { setLocationError('Unable to get location.'); setLocationLoading(false) }
        )
    }
    ...
    function resetForm() {
        ...
        setLatitude(null); setLongitude(null); setLocationError(''); setErrorMsg('')
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (environmental && !locationAddress.trim()) { setErrorMsg('Please provide a location for environmental reports'); return }
        setSaving(true)
        try {
            const supabase = createClient()
            const { data: { user } } = await supabase.auth.getUser()
            ...
            if (table === 'waste_reports') {
                ...
                await supabase.from('waste_reports').insert({
                    submitted_by: user.id, report_type: selectedCategory, description,
                    location_address: locationAddress, district: profile?.district,
                    photo_url: photoUrl, status: 'pending', latitude, longitude,
                })
            } else {
                const { data: cd, error } = await supabase.from('complaints').insert({
                    submitted_by: user.id, role: 'commercial_establishment',
                    district: profile?.district, complaint_type: selectedCategory,
            ...
            setSuccess(true); resetForm(); await loadData()
    ...
    return (
        <DashboardLayout role="Commercial" userName={profile?.full_name || profile?.organisation_name || ''} navItems={COMMERCIAL_NAV}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    ...
                    {profile?.district && (
                        <span style={{ fontSize: 12, color: '#717a6d', padding: '6px 14px', borderRadius: 99, background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                            {profile.organisation_name || profile.full_name} · {profile.district}
                        </span>
                    )}
                </div>
            </div>
                    ...
                    <form onSubmit={handleSubmit} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        {selectedCategory === 'other' && (
                            <div className="slide-in">
                                ...
                                <input className="field" placeholder="e.g. Waste left near premises…"
                        ...
                        {environmental && selectedCategory && (
                            <div className="slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '14px 16px', borderRadius: 12, background: '#fef2f2', border: '1px solid rgba(186,26,26,0.12)' }}>
                                ...
                                <div>
                                    <button type="button" onClick={getLocation} className={`loc-btn ${latitude ? 'gps-on' : ''}`}>
                        ...
                        <div>
                            ...
                            <textarea className="field" rows={4} style={{ resize: 'vertical' }}
                                placeholder={environmental
                                    ...
                                    : 'Describe the issue — what happened, when, and any relevant details…'}
