...
function StatusPill({ config, status }: { config: Record<string, any>; status: string }) {
    ...
    return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 8, fontSize: 12, fontWeight: 600, fontFamily: 'Manrope', background: sc.bg, color: sc.color }}>
            <span className="msf" style={{ fontSize: 13 }}>{sc.icon}</span>
            {sc.label}
        </span>
    )
}
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(4px)' }} onClick={onClose}>
...
export default function AdminReportsComplaintsPage() {
    ...
    async function loadAll() {
        ...
        setProfile(p)
        const { data: rData } = await supabase.from('waste_reports').select(`*, reporter:profiles!waste_reports_submitted_by_fkey(full_name, phone)`).order('created_at', { ascending: false }).limit(200)
        setReports(rData || [])
        setRLoad(false)
        const { data: cData } = await supabase.from('complaints').select(`*, complainant:profiles!complaints_user_id_fkey(full_name)`).order('created_at', { ascending: false }).limit(300)
        setComplaints(cData || [])
        setCLoad(false)
    }
    ...
    async function updateReportStatus(id: string, status: string) {
        ...
        if (!error) { setReports(prev => prev.map(r => r.id === id ? { ...r, status } : r)); setSelReport(prev => prev?.id === id ? { ...prev, status } : prev); flash('success', `Report marked as "${REPORT_STATUS_CONFIG[status]?.label || status}".`) }
    ...
    async function updateComplaintStatus(id: string, status: string) {
        ...
        if (!error) { setComplaints(prev => prev.map(c => c.id === id ? { ...c, status } : c)); setSelComp(prev => prev?.id === id ? { ...prev, status } : prev); flash('success', `Complaint status updated to "${COMPLAINT_STATUS_CONFIG[status]?.label || status}".`) }
        else flash('error', error.message)
        setUpdating(false)
    }

    function flash(type: 'success' | 'error', text: string) { setMessage({ type, text }); setTimeout(() => setMessage(null), 3500) }
    ...
    const rStats = { total: reports.length, pending: reports.filter(r => r.status === 'pending').length, resolved: reports.filter(r => r.status === 'resolved').length }
    const cStats = { total: complaints.length, open: complaints.filter(c => c.status === 'open').length, resolved: complaints.filter(c => c.status === 'resolved').length }
    const districtBreakdown = ['District 1', 'District 2', 'District 3', 'District 4'].map(d => ({ district: d, total: complaints.filter(c => c.district === d).length, open: complaints.filter(c => c.district === d && c.status === 'open').length, resolved: complaints.filter(c => c.district === d && c.status === 'resolved').length }))
            ...
            <div style={{ maxWidth: 1100, margin: '0 auto', paddingBottom: 40 }}>
                ...
                <div className="a1" style={{ marginBottom: 24 }}>
                    ...
                    <h1 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 46, fontWeight: 800, color: '#181c22', lineHeight: 1.05, margin: '0 0 4px' }}>
                        Reports &amp; <span style={{ color: '#00450d' }}>Complaints</span>
                    </h1>
                    ...
                    {[
                        ...
                        <div key={s.label} className="stat-card">
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                                <span className="msf" style={{ fontSize: 16, color: s.color }}>{s.icon}</span>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: '#f4f6f3', borderRadius: 14, padding: 6, width: 'fit-content' }}>
                    <button className={`tab-btn${activeTab === 'reports' ? ' active' : ''}`} onClick={() => setActiveTab('reports')}>
                        <span className="msf" style={{ fontSize: 16 }}>report</span>
                        Waste Reports
                        {rStats.pending > 0 && <span style={{ background: activeTab === 'reports' ? 'rgba(255,255,255,0.25)' : '#d97706', color: 'white', borderRadius: 99, fontSize: 11, padding: '1px 7px', fontWeight: 700, fontFamily: 'Manrope' }}>{rStats.pending}</span>}
                    </button>
                    <button className={`tab-btn${activeTab === 'complaints' ? ' active' : ''}`} onClick={() => setActiveTab('complaints')}>
                        <span className="msf" style={{ fontSize: 16 }}>feedback</span>
                        Complaints
                        {cStats.open > 0 && <span style={{ background: activeTab === 'complaints' ? 'rgba(255,255,255,0.25)' : '#d97706', color: 'white', borderRadius: 99, fontSize: 11, padding: '1px 7px', fontWeight: 700, fontFamily: 'Manrope' }}>{cStats.open}</span>}
                    </button>
                </div>

                {message && (
                    <div className={`toast ${message.type}`}>
                        <span className="msf" style={{ fontSize: 16 }}>{message.type === 'success' ? 'check_circle' : 'error'}</span>
                        {message.text}
                    </div>
                )}
                    ...
                    <>
                        <div style={{ background: 'white', borderRadius: 16, padding: '14px 20px', border: '1px solid rgba(0,69,13,0.06)', marginBottom: 16, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                            <div style={{ position: 'relative' }}>
                                <span className="msf" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: '#a0a89b' }}>search</span>
                                <input className="search-box" placeholder="Search reports…" value={rSearch} onChange={e => setRSearch(e.target.value)} />
                            </div>
                            <select className="filter-select" value={rFilterType} onChange={e => setRFilterType(e.target.value)}>{REPORT_TYPES.map(t => <option key={t}>{t}</option>)}</select>
                            ...
                            <select className="filter-select" value={rFilterDistrict} onChange={e => setRFilterDist(e.target.value)}>{DISTRICTS.map(d => <option key={d}>{d}</option>)}</select>
                        ...
                        <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,69,13,0.06)', overflow: 'hidden' }}>
                            ...
                            {reportsLoading ? (
                                <div style={{ padding: 40, textAlign: 'center', color: '#717a6d', fontSize: 14 }}>Loading…</div>
                            ) : filteredReports.length === 0 ? (
                                <div style={{ padding: 48, textAlign: 'center' }}>
                                    <span className="msf" style={{ fontSize: 40, color: '#d1d5db', display: 'block', marginBottom: 10 }}>report_off</span>
                                ...
                                <div key={r.id} className="row" style={{ gridTemplateColumns: '1fr 120px 120px 120px 150px' }} onClick={() => setSelReport(r)}>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: '#181c22', fontFamily: 'Manrope', marginBottom: 2 }}>{r.description?.slice(0, 60)}{(r.description?.length || 0) > 60 ? '…' : ''}</div>
                                        <div style={{ fontSize: 12, color: '#a0a89b', fontFamily: 'Inter' }}>{new Date(r.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}{r.ward ? ` · ${r.ward}` : ''}</div>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', gap: 5 }} onClick={e => e.stopPropagation()}>
                                        {r.status === 'pending' && <button className="action-btn" style={{ background: '#eff6ff', color: '#1d4ed8' }} onClick={() => updateReportStatus(r.id, 'under_review')} disabled={updating}>Review</button>}
                                        {(r.status === 'pending' || r.status === 'under_review') && <button className="action-btn" style={{ background: '#f0fdf4', color: '#166534' }} onClick={() => updateReportStatus(r.id, 'resolved')} disabled={updating}>Resolve</button>}
                                        {r.status === 'pending' && <button className="action-btn" style={{ background: '#fef2f2', color: '#dc2626' }} onClick={() => updateReportStatus(r.id, 'rejected')} disabled={updating}>Reject</button>}
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'complaints' && (
                    <>
                        ...
                        <div style={{ background: 'white', borderRadius: 16, padding: '14px 20px', border: '1px solid rgba(0,69,13,0.06)', marginBottom: 16, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                            ...
                            <select className="filter-select" value={cFilterDistrict} onChange={e => setCFilterDist(e.target.value)}>{DISTRICTS.map(d => <option key={d}>{d}</option>)}</select>
                            <select className="filter-select" value={cFilterType} onChange={e => setCFilterType(e.target.value)}>{allCTypes.map(t => <option key={t}>{t}</option>)}</select>
                        ...
                        <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(0,69,13,0.06)', overflow: 'hidden' }}>
                            ...
                            {complaintsLoading ? (
                                ...
                                <div style={{ padding: 48, textAlign: 'center' }}>
                                    <span className="msf" style={{ fontSize: 40, color: '#d1d5db', display: 'block', marginBottom: 10 }}>sentiment_satisfied</span>
                                ...
                                <div key={c.id} className="row" style={{ gridTemplateColumns: '1fr 110px 120px 110px 130px' }} onClick={() => setSelComp(c)}>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: '#181c22', fontFamily: 'Manrope', marginBottom: 2 }}>{c.description?.slice(0, 55)}{(c.description?.length || 0) > 55 ? '…' : ''}</div>
                                    ...
                                    <div style={{ display: 'flex', gap: 5 }} onClick={e => e.stopPropagation()}>
                                        {c.status === 'open' && <button className="action-btn" style={{ background: '#eff6ff', color: '#1d4ed8' }} onClick={() => updateComplaintStatus(c.id, 'in_progress')} disabled={updating}>Start</button>}
                                        {(c.status === 'open' || c.status === 'in_progress') && <button className="action-btn" style={{ background: '#f0fdf4', color: '#166534' }} onClick={() => updateComplaintStatus(c.id, 'resolved')} disabled={updating}>Resolve</button>}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div style={{ borderTop: '1px solid #f4f6f3', paddingTop: 16 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            {Object.entries(REPORT_STATUS_CONFIG).map(([k, v]) => {
                                ...
                                return <button key={k} onClick={() => updateReportStatus(selectedReport.id, k)} disabled={isCurrent || updating} style={{ border: isCurrent ? `2px solid ${v.color}` : '1.5px solid #e4e9e0', borderRadius: 10, padding: '7px 14px', fontSize: 13, fontWeight: 600, fontFamily: 'Manrope', cursor: isCurrent ? 'default' : 'pointer', background: isCurrent ? v.bg : 'white', color: isCurrent ? v.color : '#41493e', opacity: updating ? 0.6 : 1 }}>{v.label}</button>
                            })}
                        </div>
                    </div>
                </Modal>
                ...
                <Modal onClose={() => setSelComp(null)}>
                    ...
                    {selectedComplaint.tx_hash && (
                        <div style={{ background: '#eff6ff', borderRadius: 10, padding: '10px 14px', marginBottom: 16, display: 'flex', gap: 10, alignItems: 'center' }}>
                            <span className="msf" style={{ fontSize: 16, color: '#1d4ed8' }}>link</span>
                    ...
                    <div style={{ borderTop: '1px solid #f4f6f3', paddingTop: 16 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            {Object.entries(COMPLAINT_STATUS_CONFIG).map(([k, v]) => {
                                ...
                                return <button key={k} onClick={() => updateComplaintStatus(selectedComplaint.id, k)} disabled={isCurrent || updating} style={{ border: isCurrent ? `2px solid ${v.color}` : '1.5px solid #e4e9e0', borderRadius: 10, padding: '7px 14px', fontSize: 13, fontWeight: 600, fontFamily: 'Manrope', cursor: isCurrent ? 'default' : 'pointer', background: isCurrent ? v.bg : 'white', color: isCurrent ? v.color : '#41493e', opacity: updating ? 0.6 : 1 }}>{v.label}</button>
                            })}
                        </div>
                    </div>
                </Modal>
