...
interface CommercialSummary {
    ...
    total_bins_this_period: number
    bins_this_week: number
...
export default function AdminBillingPage() {
    ...
    async function loadData() {
        ...
        const { data: ratesData } = await supabase
            .from('billing_rates').select('*').order('effective_from', { ascending: false })
        setRates(ratesData || [])
        ...
        const enriched = (summaryData || []).map((s: CommercialSummary) => {
            const rate = s.current_tier === 'tier_b'
                ...
                : ratesData?.find(r => r.tier === 'tier_a')?.rate_per_bin || 0
            const hasInvoice = (invoicesData || []).some(inv =>
                inv.commercial_id === s.commercial_id &&
                new Date(inv.period_start).getMonth() === new Date().getMonth()
            )
            return {
                ...s,
                estimated_amount: s.total_bins_this_period * rate,
        ...
        for (const id of selectedCommercials) {
            try {
                const res = await fetch('/api/billing/generate-invoices', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ commercial_id: id }),
                })
                const data = await res.json()
    ...
    async function reviewCycleRequest(requestId: string, action: 'approved' | 'rejected', commercialId: string, newCycle: string) {
        ...
        await supabase.from('billing_cycle_requests').update({
            status: action, reviewed_by: user?.id,
            reviewed_at: new Date().toISOString(), effective_from: effectiveFrom,
        }).eq('id', requestId)
        if (action === 'approved') {
            await supabase.from('profiles').update({
                billing_cycle: newCycle, billing_cycle_effective_from: effectiveFrom,
            }).eq('id', commercialId)
        }
    ...
    return (
        ...
        .gen-btn {
          ...
          font-family: 'Manrope', sans-serif; cursor: pointer; transition: all 0.2s ease;
          border: none; background: #00450d; color: white; white-space: nowrap;
          display: flex; align-items: center; gap: 5px;
        }
        .gen-btn:hover { background: #1b5e20; }
        .gen-btn:disabled { opacity: 0.5; cursor: not-allowed; }
        .checkbox {
          width: 18px; height: 18px; border-radius: 5px; border: 2px solid #e5e7eb;
      ...
      `}</style>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8 s2">
                {[
                    ...
                    { label: 'Pending Requests', value: pendingRequests, icon: 'swap_horiz', color: '#d97706', bg: '#fefce8' },
                ].map(m => (
                    ...
                    <div className="px-6 py-5 flex flex-wrap items-center justify-between gap-3"
                        ...
                        <div>
                            <h3 className="font-headline font-bold text-xl" style={{ color: '#181c22' }}>
                                Collection Activity — {new Date().toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })}
                            </h3>
                            <p className="text-sm mt-1" style={{ color: '#717a6d' }}>
                                Bin counts per commercial establishment this period. Select to generate invoices.
                            </p>
                        </div>
                        {eligibleForInvoice.length > 0 && (
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                <button onClick={toggleSelectAll}
                                    style={{ padding: '8px 16px', borderRadius: '99px', border: '1.5px solid rgba(0,69,13,0.2)', background: 'white', color: '#00450d', fontSize: '12px', fontWeight: 700, fontFamily: 'Manrope, sans-serif', cursor: 'pointer' }}>
                    ...
                    {loading ? (
                        ...
                        <div>
                                ...
                                <div style={{ padding: '12px 24px', background: '#f9f9ff', borderBottom: '1px solid rgba(0,69,13,0.04)', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                                    {tierARate && (
                                        <span style={{ fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <span className="badge" style={{ background: '#f0fdf4', color: '#00450d' }}>Tier A</span>
                                            0–30 bins · LKR {tierARate.rate_per_bin}/bin
                                        </span>
                                    )}
                                    {tierBRate && (
                                        <span style={{ fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <span className="badge" style={{ background: '#eff6ff', color: '#1d4ed8' }}>Tier B</span>
                                            31–100 bins · LKR {tierBRate.rate_per_bin}/bin
                                        </span>
                                    )}
                                    {!tierARate && !tierBRate && (
                                        <span style={{ fontSize: '12px', color: '#ba1a1a', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>warning</span>
                                            No billing rates set — go to Billing Rates tab first
                                        </span>
                                    )}
                                </div>
                            )}
                            {commercialSummaries.map(s => {
                                ...
                                const tierStyle = s.current_tier === 'tier_b'
                                    ...
                                    <div key={s.commercial_id} className="row"
                                            ...
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', fontSize: '12px', color: '#94a3b8' }}>
                                                ...
                                                {s.last_collection && (
                                                    <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                                                        ...
                                                        Last: {new Date(s.last_collection).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0 }}>
                                            {s.total_bins_this_period > 0 ? (
                                                <>
                                                    <p style={{ fontSize: '16px', fontWeight: 800, color: '#181c22', fontFamily: 'Manrope, sans-serif' }}>
                                                        LKR {(s.estimated_amount || 0).toLocaleString()}
                                                    </p>
                                                    <p style={{ fontSize: '10px', color: '#94a3b8', margin: 0 }}>estimated</p>
                                                </>
                                            ) : (
                                                <p style={{ fontSize: '13px', color: '#94a3b8' }}>No collections</p>
                                            )}
                                            {isEligible && (
                                                <button className="gen-btn" disabled={generatingFor === s.commercial_id}
                                                    onClick={e => { e.stopPropagation(); generateForCommercial(s.commercial_id) }}>
                ...
                <div className="bento-card">
                    ...
                    {invoices.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16">
                            ...
                            <p style={{ color: '#94a3b8', fontSize: '12px', marginTop: '4px' }}>Go to Collection Overview to generate invoices</p>
                        </div>
                    ) : (
                        <div>
                                    ...
                                    <div key={invoice.id} className="row">
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                                ...
                                                <span className="badge" style={{ background: invoice.tier === 'tier_b' ? '#eff6ff' : '#f0fdf4', color: invoice.tier === 'tier_b' ? '#1d4ed8' : '#00450d' }}>
                                                    {invoice.tier === 'tier_b' ? 'Tier B' : 'Tier A'}
                                                </span>
                                            </div>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '12px', color: '#94a3b8' }}>
                                                ...
                                                <span>
                                                    {new Date(invoice.period_start).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} —{' '}
                                                    {new Date(invoice.period_end).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                                                </span>
            ...
            {activeTab === 'rates' && (
                <div className="bento-card">
                    <div className="px-8 py-6" style={{ borderBottom: '1px solid rgba(0,69,13,0.06)' }}>
                        ...
                        <p className="text-sm mt-1" style={{ color: '#717a6d' }}>
                            New rates apply from the effective date. Previous rates are kept for audit.
                        </p>
                    </div>
                    <form onSubmit={saveRates} className="p-8">
                        ...
                        <button type="submit" disabled={savingRates} className="submit-btn">
                            {savingRates ? 'Saving...' : (
                                <><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>save</span>Save Rates</>
                            )}
                        </button>
                    </form>
                    {rates.length > 0 && (
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                            ...
                                            <span className="badge" style={{
                                                ...
                                                color: req.status === 'pending' ? '#d97706' : req.status === 'approved' ? '#00450d' : '#ba1a1a'
                                            }}>{req.status}</span>
                                        </div>
                                        <p style={{ fontSize: '13px', color: '#64748b' }}>
                                            <span style={{ textTransform: 'capitalize' }}>{req.current_cycle}</span>
                                            {' → '}
                                            <strong style={{ textTransform: 'capitalize' }}>{req.requested_cycle}</strong>
                                    ...
                                    {req.status === 'pending' && (
                                        <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                                            <button onClick={() => reviewCycleRequest(req.id, 'approved', req.commercial_id, req.requested_cycle)}
                                                className="action-btn" style={{ borderColor: 'rgba(0,69,13,0.2)', color: '#00450d', background: 'white' }}>
                                                Approve
                                            </button>
                                            <button onClick={() => reviewCycleRequest(req.id, 'rejected', req.commercial_id, req.requested_cycle)}
                                                className="action-btn" style={{ borderColor: 'rgba(186,26,26,0.2)', color: '#ba1a1a', background: 'white' }}>
                                                Reject
                                            </button>
                                        </div>
                                    )}
