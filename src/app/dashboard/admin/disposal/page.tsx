...
export default function AdminDisposalPage() {
    ...
    async function loadData() {
        ...
        if (recs.length > 0) {
            ...
            const enriched = recs.map((r: any) => ({ ...r, driver: dProfiles?.find((d: any) => d.id === r.driver_id) || null, contractor: cProfiles?.find((c: any) => c.id === r.contractor_id) || null }))
            setRecords(enriched)
            setDistricts([...new Set(enriched.map((r: any) => r.district).filter(Boolean))] as string[])
        } else { setRecords([]) }
        ...
        if (discs.length > 0) {
            ...
            setDiscrepancies(discs.map((d: any) => ({ ...d, driver: dProfiles?.find((p: any) => p.id === d.driver_id) || null, contractor: cProfiles?.find((p: any) => p.id === d.contractor_id) || null })))
    ...
    const filteredRecords = records.filter(r => {
        ...
        if (search) { const q = search.toLowerCase(); return (r.facility_name || '').toLowerCase().includes(q) || (r.driver?.full_name || '').toLowerCase().includes(q) || (r.contractor?.organisation_name || '').toLowerCase().includes(q) || (r.district || '').toLowerCase().includes(q) }
        return true
    })
    ...
    return (
      ...
      `}</style>
                                ...
                                <div key={record.id} className="row" style={{ cursor: 'pointer' }} onClick={() => setSelectedRecord(record)}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                                            ...
                                            <span className="badge" style={{ background: rs.bg, color: rs.color }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: rs.dot, display: 'inline-block' }} />{rs.label}</span>
                        ...
                        <div key={s.id} className="row">
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                                    ...
                                    <span className="badge" style={{ background: s.published ? '#f0fdf4' : '#fefce8', color: s.published ? '#00450d' : '#d97706' }}>{s.published ? 'Published' : 'Draft'}</span>
                            ...
                            <span className="badge" style={{ background: s.status === 'completed' ? '#f0fdf4' : s.status === 'in_transit' ? '#fefce8' : s.status === 'cancelled' ? '#fef2f2' : '#eff6ff', color: s.status === 'completed' ? '#00450d' : s.status === 'in_transit' ? '#d97706' : s.status === 'cancelled' ? '#ba1a1a' : '#1d4ed8' }}>
            ...
            {/* ── Record detail modal ── */}
            {selectedRecord && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, backdropFilter: 'blur(4px)' }}
                    onClick={() => setSelectedRecord(null)}>
                    <div className="card" style={{ width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', padding: 28 }} onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => setSelectedRecord(null)} style={{ width: '100%', padding: 12, borderRadius: 10, border: '1.5px solid #e5e7eb', background: 'white', fontFamily: 'Manrope,sans-serif', fontWeight: 600, fontSize: 14, cursor: 'pointer', color: '#64748b' }}>
                            Close
                        </button>
                    </div>
                </div>
