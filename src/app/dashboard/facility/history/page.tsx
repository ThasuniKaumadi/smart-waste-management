...
interface WeeklyTonnage {
    week: string; district: string; waste_type: string
    total_quantity: number; unit: string; intake_count: number; rejection_count: number
}
...
export default function FacilityHistoryPage() {
    ...
    const groupedByWeek = filteredWeekly.reduce((acc, row) => {
        ...
        acc[row.week].push(row); return acc
    }, {} as Record<string, WeeklyTonnage[]>)

    const wasteTypeColor: Record<string, { color: string; bg: string }> = {
        organic: { color: '#15803d', bg: '#f0fdf4' }, recyclable: { color: '#1d4ed8', bg: '#eff6ff' },
        'non-recyclable': { color: '#d97706', bg: '#fefce8' }, hazardous: { color: '#ba1a1a', bg: '#fef2f2' },
    ...
    function conditionStyle(c: string) {
        if (c === 'segregated') return { background: '#f0fdf4', color: '#00450d' }
        if (c === 'mixed') return { background: '#fefce8', color: '#92400e' }
        if (c === 'contaminated') return { background: '#fef2f2', color: '#ba1a1a' }
    ...
    return (
        ...
        .filter-btn.active{background:#0369a1;color:white;}
        ...
        .tx-chip{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:6px;background:#f0f9ff;border:1px solid rgba(3,105,161,0.12);font-size:10px;font-family:'Courier New',monospace;color:#0369a1;cursor:pointer;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-decoration:none;}
      ...
      `}</style>
            ...
            <section className="s1" style={{ marginBottom: 32 }}>
                <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.18em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', textTransform: 'uppercase', margin: '0 0 8px' }}>Facility Operator · Intake Records</p>
                <h1 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 40, fontWeight: 900, color: '#181c22', lineHeight: 1.1, margin: 0 }}>
                    Intake <span style={{ color: '#0369a1' }}>History</span>
                </h1>
            </section>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 28 }} className="s2">
                        {[
                            { label: 'Total Intakes', value: intakes.length, icon: 'inventory', color: '#0369a1' },
                            { label: 'Total Weight', value: `${totalWeight.toFixed(1)} kg`, icon: 'scale', color: '#15803d' },
                                    ...
                                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        <thead>
                                            <tr style={{ background: '#f9f9ff' }}>
                                                {['Date', 'Vehicle', 'Waste Type', 'Method', 'Quantity', 'Condition', 'Status', 'Blockchain'].map(h => (
                                        ...
                                        <tbody>
                                            {filtered.map(log => (
                                                <tr key={log.id} style={{ borderTop: '1px solid rgba(0,69,13,0.04)' }}>
                                                    ...
                                                    <td style={{ padding: '12px 16px' }}><p style={{ fontSize: 13, color: '#64748b', textTransform: 'capitalize', margin: 0 }}>{log.processing_method || '—'}</p></td>
                                    ...
                                    <div key={week} style={{ marginBottom: 24 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                <div style={{ width: 3, height: 24, background: '#0369a1', borderRadius: 99 }} />
                                                            ...
                                                            <tr key={idx} style={{ borderTop: '1px solid rgba(0,69,13,0.04)' }}>
                                                                ...
                                                                <td style={{ padding: '12px 16px' }}>
                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                                        <div style={{ flex: 1, height: 6, background: '#f0f9ff', borderRadius: 99, overflow: 'hidden', minWidth: 60 }}>
                                                                            <div style={{ height: '100%', width: `${share}%`, background: '#0369a1', borderRadius: 99 }} />
                                                                        </div>
                                                                        <span style={{ fontSize: 12, fontWeight: 700, color: '#0369a1', fontFamily: 'Manrope,sans-serif', minWidth: 36 }}>{share.toFixed(0)}%</span>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                        )
                                                    })}
                                                    <tr style={{ background: '#f0f9ff', borderTop: '2px solid rgba(3,105,161,0.1)' }}>
                                                        <td style={{ padding: '10px 16px' }} colSpan={2}><p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: '#0369a1', fontFamily: 'Manrope,sans-serif', letterSpacing: '0.1em', margin: 0 }}>Week Total</p></td>
                                                        <td style={{ padding: '10px 16px' }}><p style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 900, fontSize: 13, color: '#0369a1', margin: 0 }}>{weekTotal.toFixed(1)} kg</p></td>
                                                        <td style={{ padding: '10px 16px' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#0369a1', margin: 0 }}>{rows.reduce((s, r) => s + r.intake_count, 0)}</p></td>
                                                        <td style={{ padding: '10px 16px' }}><p style={{ fontSize: 11, fontWeight: 700, color: '#0369a1', margin: 0 }}>100%</p></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
