...
export default function FacilityAnalyticsPage() {
    ...
    async function loadData() {
        ...
        allLogs.forEach((log: any) => {
            ...
            if (!monthMap[key]) monthMap[key] = { month: key, label: lbl, totalWeight: 0, totalAmount: 0, intakeCount: 0, rejectionCount: 0 }
    ...
    function buildMaterials(logs: any[], month: string) {
        ...
        filtered.forEach((log: any) => {
            const mat = log.material_type || log.waste_type || 'other'
    ...
    return (
      ...
      `}</style>
            ...
            <div className="a1" style={{ marginBottom: 24 }}>
                <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.18em', color: '#94a3b8', fontFamily: 'Manrope,sans-serif', textTransform: 'uppercase', marginBottom: 8 }}>Facility Operator · Analytics</p>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    <h1 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 38, fontWeight: 900, color: '#181c22', lineHeight: 1.05, margin: 0 }}>
                        Intake <span style={{ color: '#0369a1' }}>Analytics</span>
                    </h1>
            ...
            {loading ? (
                <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0' }}>
                    <div style={{ width: 26, height: 26, border: '2px solid #0369a1', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .8s linear infinite' }} />
                </div>
            ) : (
                <>
                    {/* Summary stats */}
                    <div className="a2" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
                        {[
                            { label: 'Volume Received', value: `${currentData.totalWeight.toFixed(1)} kg`, icon: 'scale', color: '#0369a1', bg: '#f0f9ff' },
                            { label: 'Total Intakes', value: currentData.intakeCount, icon: 'inventory', color: '#15803d', bg: '#f0fdf4' },
                            { label: 'Rejections', value: totalRejections, icon: 'cancel', color: '#ba1a1a', bg: '#fef2f2' },
                            { label: 'Chain Verified', value: intakes.filter(l => l.tx_hash).length, icon: 'verified', color: '#0e7490', bg: '#ecfeff' },
                        ].map(m => (
                            <div key={m.label} className="card" style={{ padding: '16px 18px' }}>
                                <div style={{ width: 34, height: 34, borderRadius: 9, background: m.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
                                    <span className="msf" style={{ fontSize: 16, color: m.color }}>{m.icon}</span>
                                </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                                        ...
                                        return (
                                            <div key={m.month} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}
                                                ...
                                                <div style={{ flex: 1, height: 22, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                                                    <div style={{ height: '100%', width: `${barW}%`, background: isSelected ? '#0369a1' : '#bae6fd', borderRadius: 6, transition: 'width 0.4s ease', minWidth: barW > 0 ? 4 : 0 }} />
                                                </div>
                                                <span style={{ fontSize: 11, fontWeight: 700, color: isSelected ? '#0369a1' : '#64748b', fontFamily: 'Manrope,sans-serif', width: 52, textAlign: 'right', flexShrink: 0 }}>
                        ...
                        {/* Monthly intakes chart */}
                        <div className="a3 card" style={{ padding: '20px 22px' }}>
                            <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, color: '#181c22', margin: '0 0 16px' }}>Monthly Intake Count</h3>
                            {monthly.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}>
                                    <span className="msf" style={{ fontSize: 28, color: '#d1d5db', display: 'block', marginBottom: 8 }}>inventory</span>
                                    <p style={{ fontSize: 12, color: '#94a3b8' }}>No data yet</p>
                                </div>
                            ) : (() => {
                                const maxCount = Math.max(...monthly.map(m => m.intakeCount), 1)
                                return (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {monthly.slice(0, 6).map(m => {
                                            const barW = Math.round((m.intakeCount / maxCount) * 100)
                                            const isSelected = selectedMonth === m.month
                                            return (
                                                <div key={m.month} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}
                                                    onClick={() => handleMonthChange(isSelected ? 'all' : m.month)}>
                                                    <span style={{ fontSize: 11, color: '#64748b', width: 56, flexShrink: 0, fontFamily: 'Manrope,sans-serif', fontWeight: isSelected ? 700 : 400 }}>{m.label}</span>
                                                    <div style={{ flex: 1, height: 22, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                                                        <div style={{ height: '100%', width: `${barW}%`, background: isSelected ? '#15803d' : '#bbf7d0', borderRadius: 6, transition: 'width 0.4s ease', minWidth: barW > 0 ? 4 : 0 }} />
                                                    </div>
                                                    <span style={{ fontSize: 11, fontWeight: 700, color: isSelected ? '#15803d' : '#64748b', fontFamily: 'Manrope,sans-serif', width: 52, textAlign: 'right', flexShrink: 0 }}>
                                                        {m.intakeCount}
                                                    </span>
                                                </div>
                                            )
                    ...
                    <div className="a4 card">
                        <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(0,69,13,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <h3 style={{ fontFamily: 'Manrope,sans-serif', fontWeight: 700, fontSize: 14, color: '#181c22', margin: 0 }}>Waste Type Breakdown</h3>
                            <span style={{ fontSize: 12, color: '#94a3b8' }}>{selectedMonth === 'all' ? 'All time' : monthly.find(m => m.month === selectedMonth)?.label}</span>
                        </div>
                        {materials.length === 0 ? (
                            <div style={{ padding: '50px 24px', textAlign: 'center' }}>
                                <span className="msf" style={{ fontSize: 36, color: '#d1d5db', display: 'block', marginBottom: 12 }}>delete_sweep</span>
                                ...
                                return (
                                    <div key={mat.material} style={{ padding: '14px 18px', borderBottom: '1px solid rgba(0,69,13,0.05)', display: 'flex', alignItems: 'center', gap: 14 }}>
                                        <div style={{ width: 36, height: 36, borderRadius: 10, background: style.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                            <span className="msf" style={{ fontSize: 18, color: style.color }}>delete_sweep</span>
                                        </div>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    ...
                                                    <span className="badge" style={{ background: style.bg, color: style.color }}>{mat.count} intakes</span>
                                                </div>
                                                <span style={{ fontSize: 12, fontWeight: 700, color: style.color, fontFamily: 'Manrope,sans-serif' }}>{mat.totalWeight.toFixed(1)} kg</span>
                                            </div>
                                            <div style={{ height: 6, background: '#f1f5f9', borderRadius: 99, overflow: 'hidden' }}>
                                                <div style={{ height: '100%', width: `${share}%`, background: style.color, borderRadius: 99, opacity: 0.7 }} />
                                            </div>
                                        </div>
                        ...
                        {materials.length > 0 && (
                            <div style={{ padding: '12px 18px', background: '#f9fbf9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: 13, fontWeight: 700, color: '#0369a1', fontFamily: 'Manrope,sans-serif' }}>
